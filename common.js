...
async function fetchNotices(){
  try{
    ...
    if(res.ok){const txt=(await res.text()).trim();if(txt){const data=JSON.parse(txt);
      ...
      if(data&&Array.isArray(data.notices)){saveNoticesLocal(data.notices);try{localStorage.setItem('notices-homecount',String(data.homeCount!=null?data.homeCount:2))}catch(_e){}try{localStorage.setItem('notices-show',JSON.stringify(data.show||{}))}catch(_e){}return data.notices} // 新格式
    }}
  }catch(_e){}
...
async function publishNotices(homeCount,listCount,show,onStep){
  ...
  let r;
  const sh=show||{};
  try{r=await gasCall('publishNotices',{homeCount:homeCount,listCount:listCount,showCat:sh.cat?1:'',showSubcat:sh.subcat?1:'',showTags:sh.tags?1:''},40000);}catch(e){return {ok:false,error:'gas exception:'+e,step:'連GAS'};}
...
async function gasApproveAction(seq,code){
  const r=await gasCallPost('approveAction',{seq,code},20000);
  return r||{ok:false,error:'GAS無回應'};
}
...
global.MP={
  ...
  newSlip,slipUnitsTotal,slipLaodianTotal,PRESS_LEVELS,BODY_PARTS,CLIENT_REQS,custKey,loadCustDB,saveCustDB,getCust,getGasUrl,setGasUrl,getBuiltinGasUrl,shouldClaimKey,autoClaimKey,diagClaimKey,gasCall,gasCallPost,gasWarmup,getNoticesLocal,saveNoticesLocal,fetchNotices,getNoticeHomeCount,getNoticeShow,publishNotices,noticeBody,noticeTitle,noticeSummary,noticeCats,noticeTags,getMyKey,setMyKey,hasMyKey,isNoticeRead,markNoticeRead,getNoticeReadCount,getNoticeReaders,getNoticeUnread,gasTranslate,gasAnalyze,gasUsage,gasAddNotice,gasEditNotice,gasSubmitAction,gasCheckAction,gasApproveAction,gasUpdatePwd,gasLoginPwd,gasSetInitialPwd,gasSyncProfile,gasCheckCode,getAdminSecret,setAdminSecret,hasAdminSecret,issueKey,claimMyKey,upsertCust,deleteCust,searchCustDB,recentCust,custLastSlip,migrateDayGroups,migrateMonthGroups,slipSvcLabel,slipStartTime,loadTagHistory,addTagHistory,visitStats,collectSlips,collectAllSlips,tagStats,searchSlips,
