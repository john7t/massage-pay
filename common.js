/* ════════════════════════════════════════════════════════════
   ...
   v1.9-003 / 首頁今日標題(綠框)點了跳DayEditor小視窗編輯(複用前一筆的編輯器);移除上批的首頁今日流水列表(避免重複);今日與前一筆編輯體驗統一
...
function newSlip(units){const now=Date.now();return{id:'S'+now+Math.floor(Math.random()*1000),units:units||1,laodian:0,createdAt:now,startAt:0,tags:[],custName:'',custTitle:'',custPhone:'',svc:'',extra:0}}
// 流水支數/老點加總
...
global.MP={
  ...
  newSlip,slipUnitsTotal,slipLaodianTotal,migrateDayGroups,migrateMonthGroups,slipSvcLabel,slipStartTime,loadTagHistory,addTagHistory,visitStats,collectSlips,collectAllSlips,tagStats,searchSlips,
