...
// 公告:從GAS讀取存本機(這階段不驗證)
function getNoticesLocal(){try{return LS.get('notices-cache')||[]}catch(_e){return []}}
function saveNoticesLocal(list){try{LS.set('notices-cache',list||[])}catch(_e){}}
async function fetchNotices(){
  const r=await gasCall('notices',{});
  if(r&&r.ok&&Array.isArray(r.notices)){saveNoticesLocal(r.notices);try{localStorage.setItem('notices-synced',String(Date.now()))}catch(_e){}return r.notices}
...
global.MP={
  ...
  newSlip,slipUnitsTotal,slipLaodianTotal,PRESS_LEVELS,BODY_PARTS,CLIENT_REQS,custKey,loadCustDB,saveCustDB,getCust,getGasUrl,setGasUrl,getBuiltinGasUrl,shouldClaimKey,autoClaimKey,gasCall,gasWarmup,getNoticesLocal,saveNoticesLocal,fetchNotices,noticeBody,noticeCats,noticeTags,getMyKey,setMyKey,hasMyKey,getAdminSecret,setAdminSecret,hasAdminSecret,issueKey,claimMyKey,upsertCust,deleteCust,searchCustDB,recentCust,custLastSlip,migrateDayGroups,migrateMonthGroups,slipSvcLabel,slipStartTime,loadTagHistory,addTagHistory,visitStats,collectSlips,collectAllSlips,tagStats,searchSlips,
