...
async function fetchNotices(){
  try{
    ...
    if(res.ok){const txt=(await res.text()).trim();if(txt){const data=JSON.parse(txt);
      ...
      if(data&&Array.isArray(data.notices)){saveNoticesLocal(data.notices);try{localStorage.setItem('notices-homecount',String(data.homeCount!=null?data.homeCount:2))}catch(_e){}return data.notices} // 新格式
    }}
  }catch(_e){}
...
async function publishNotices(homeCount,listCount,onStep){
  ...
  let r;
  try{r=await gasCall('publishNotices',{homeCount:homeCount,listCount:listCount},40000);}catch(e){return {ok:false,error:'gas exception:'+e,step:'連GAS'};}
...
global.MP={
  ...
  newSlip,slipUnitsTotal,slipLaodianTotal,PRESS_LEVELS,BODY_PARTS,CLIENT_REQS,custKey,loadCustDB,saveCustDB,getCust,getGasUrl,setGasUrl,getBuiltinGasUrl,shouldClaimKey,autoClaimKey,diagClaimKey,gasCall,gasWarmup,getNoticesLocal,saveNoticesLocal,fetchNotices,getNoticeHomeCount,publishNotices,noticeBody,noticeTitle,noticeSummary,noticeCats,noticeTags,getMyKey,setMyKey,hasMyKey,isNoticeRead,markNoticeRead,getNoticeReadCount,getNoticeReaders,getAdminSecret,setAdminSecret,hasAdminSecret,issueKey,claimMyKey,upsertCust,deleteCust,searchCustDB,recentCust,custLastSlip,migrateDayGroups,migrateMonthGroups,slipSvcLabel,slipStartTime,loadTagHistory,addTagHistory,visitStats,collectSlips,collectAllSlips,tagStats,searchSlips,
