...
async function publishNotices(gh,limit){
  if(!gh||!gh.token||!gh.owner||!gh.repo)return {ok:false,error:'no github'};
  const r=await gasCall('notices',{});
  if(!(r&&r.ok&&Array.isArray(r.notices)))return {ok:false,error:'gas fail'};
  let list=r.notices;
  const n=limit||2;
  if(list.length>n)list=list.slice(-n); // 取最新N筆(尾端)
  const json=JSON.stringify(list);
  const file=await ghReadFile(gh,'notices.json');
  const ok=file?await ghWriteFile(gh,'notices.json',json,file.sha,'update notices'):await ghWriteFile(gh,'notices.json',json,undefined,'create notices');
  if(ok){saveNoticesLocal(list);return {ok:true,count:list.length}}
...
global.MP={
  ...
  newSlip,slipUnitsTotal,slipLaodianTotal,PRESS_LEVELS,BODY_PARTS,CLIENT_REQS,custKey,loadCustDB,saveCustDB,getCust,getGasUrl,setGasUrl,getBuiltinGasUrl,shouldClaimKey,autoClaimKey,gasCall,gasWarmup,getNoticesLocal,saveNoticesLocal,fetchNotices,publishNotices,noticeBody,noticeTitle,noticeSummary,noticeCats,noticeTags,getMyKey,setMyKey,hasMyKey,getAdminSecret,setAdminSecret,hasAdminSecret,issueKey,claimMyKey,upsertCust,deleteCust,searchCustDB,recentCust,custLastSlip,migrateDayGroups,migrateMonthGroups,slipSvcLabel,slipStartTime,loadTagHistory,addTagHistory,visitStats,collectSlips,collectAllSlips,tagStats,searchSlips,
