...
async function fetchNotices(){
  try{
    ...
    if(res.ok){const txt=(await res.text()).trim();if(txt){const data=JSON.parse(txt);
      ...
      if(data&&Array.isArray(data.notices)){saveNoticesLocal(data.notices);try{localStorage.setItem('notices-homecount',String(data.homeCount!=null?data.homeCount:2))}catch(_e){}try{localStorage.setItem('notices-show',JSON.stringify(data.show||{}))}catch(_e){}try{localStorage.setItem('notices-listtext',data.listText||'summary')}catch(_e){}try{localStorage.setItem('notices-counttype',data.countType||'people')}catch(_e){}return data.notices} // 新格式
    }}
  }catch(_e){}
...
async function gasCreateDisasterSurvey(name,startDate,code){
  const r=await gasCallPost('createDisasterSurvey',{name,startDate,code},15000);
  return r||{ok:false,error:'GAS無回應'};
}
...
async function gasAnalyze(content,code,feature){
  const r=await gasCall('analyzeNotice',{content:content,code:code||'',feature:feature||'分析測試'},40000);
  return r||{ok:false,error:'GAS無回應'};
}
...
global.MP={
  ...
  newSlip,slipUnitsTotal,slipLaodianTotal,PRESS_LEVELS,BODY_PARTS,CLIENT_REQS,custKey,loadCustDB,saveCustDB,getCust,getGasUrl,setGasUrl,getBuiltinGasUrl,shouldClaimKey,autoClaimKey,diagClaimKey,gasCall,gasCallPost,gasWarmup,getNoticesLocal,saveNoticesLocal,fetchNotices,getNoticeHomeCount,getNoticeShow,getNoticeListText,getNoticeCountType,publishNotices,noticeBody,noticeTitle,noticeSummary,noticeCats,noticeTags,getMyKey,setMyKey,hasMyKey,isNoticeRead,markNoticeRead,getNoticeReadCount,getNoticeReaders,getNoticeUnread,gasTranslate,gasAnalyze,gasUsage,gasAddNotice,gasEditNotice,gasSubmitAction,gasCheckAction,gasApproveAction,gasUpdatePwd,gasLoginPwd,gasSetInitialPwd,gasSyncProfile,gasCheckCode,gasRevokeSup,gasRejectAction,gasVerifyKey,gasLogNoticeOpen,gasLeaveTeacher,gasToggleNoticeStatus,gasLogDailyCheck,gasCreateGroupBuy,gasListGroupBuys,gasJoinGroupBuy,gasMyGroupBuyOrders,gasSubmitDisasterReport,gasDeclineGroupBuy,gasLogGroupBuyOpen,gasCloseGroupBuy,gasGroupBuyDetail,gasCreateDisasterSurvey,gasEndDisasterSurvey,gasListDisasterSurveys,gasMyDisasterReports,gasDisasterDayStatus,getAdminSecret,setAdminSecret,hasAdminSecret,issueKey,claimMyKey,upsertCust,deleteCust,searchCustDB,recentCust,custLastSlip,migrateDayGroups,migrateMonthGroups,slipSvcLabel,slipStartTime,loadTagHistory,addTagHistory,visitStats,collectSlips,collectAllSlips,tagStats,searchSlips,
