...
async function gasBlacklistSubmit(teacherCode,custName,custTitle,custPhone,reason,svc,startAt){
  const r=await gasCallPost('blacklistSubmit',{teacherCode,custName,custTitle,custPhone,reason,svc,startAt},15000);
  return r||{ok:false,error:'GAS無回應'};
}
...
// 雲端備份:直接用編號存取,換手機後查詢就能還原,不用行為代碼/LINE分享
async function gasBackupSubmit(code,year,jsonData){
  const r=await gasCallPost('backupSubmit',{code,year,jsonData},20000);
  return r||{ok:false,error:'GAS無回應'};
}
async function gasBackupCheck(code){
  const r=await gasCall('backupCheck',{code},10000);
  return r||{ok:false,error:'GAS無回應'};
}
async function gasBackupFetch(code){
  const r=await gasCall('backupFetch',{code},20000);
  return r||{ok:false,error:'GAS無回應'};
}
...
async function gasSetGroupBuyStatus(buyId,code,status){
  const r=await gasCallPost('setGroupBuyStatus',{buyId,code,status},15000);
  return r||{ok:false,error:'GAS無回應'};
}
...
// 穴道知識庫:依部位分類(ACUPOINT_BODYPARTS)+經脈+同身寸位置描述(不靠圖片)/保健用途/按摩手法/注意事項,keywords供搜尋比對用
// 目前只有中文,越南文欄位先留空字串,之後補上即可,前端會自動fallback顯示中文
const ACUPOINT_BODYPARTS=[
  {id:'leg',zh:'足部與小腿',vi:''},
  {id:'shoulder',zh:'肩膀',vi:''},
  {id:'upperback',zh:'上背部',vi:''},
  {id:'lowerback',zh:'下背部（腰骶部）',vi:''},
  {id:'hip',zh:'臀部與大腿',vi:''},
  {id:'arm',zh:'手臂',vi:''},
  {id:'head',zh:'頭部',vi:''},
];
...
const ACUPOINTS=[
  {id:'leg_1',bodyPart:'leg',
    name:{zh:'湧泉穴',vi:''},
    meridian:{zh:'足少陰腎經',vi:''},
    keywords:{zh:['滋陰補腎','寧心安神','緩解下肢疲勞與改善睡眠品質','湧泉穴','湧泉'],vi:[]},
    location:{zh:'位於足底部，蜷足時足底前部凹陷處（約當足底第2、3趾趾縫紋頭端與足跟連線的前三分之一與後三分之二交點上）',vi:''},
    benefit:{zh:'滋陰補腎、寧心安神、緩解下肢疲勞與改善睡眠品質',vi:''},
    technique:{zh:'用大拇指指腹或指關節由輕到重按壓，或以手掌擦熱足心',vi:''},
    caution:{zh:'孕婦及經期女性應避免強力刺激或針灸；足底有傷口或感染時暫緩按壓',vi:''}},
  {id:'leg_2',bodyPart:'leg',
    name:{zh:'太衝穴',vi:''},
    meridian:{zh:'足厥陰肝經',vi:''},
    keywords:{zh:['疏肝理氣','平肝熄風','舒緩情緒壓力與足背緊繃','太衝穴','太衝'],vi:[]},
    location:{zh:'位於足背側，當第一、二趾結合部前方凹陷處（約當趾縫紋頭端向後一寸）',vi:''},
    benefit:{zh:'疏肝理氣、平肝熄風、舒緩情緒壓力與足背緊繃',vi:''},
    technique:{zh:'以大拇指指腹垂直向下按壓，並做順時針或逆時針的順氣揉動',vi:''},
    caution:{zh:'孕婦宜平緩按摩，不宜過度強力重壓以免引發宮縮',vi:''}},
  {id:'leg_3',bodyPart:'leg',
    name:{zh:'太溪穴',vi:''},
    meridian:{zh:'足少陰腎經',vi:''},
    keywords:{zh:['滋補腎陰','強腰膝','改善下肢冰冷與足踝僵硬','太溪穴','太溪'],vi:[]},
    location:{zh:'位於足內側，內踝尖與跟腱之間的凹陷處（內踝尖與阿基里斯腱中點）',vi:''},
    benefit:{zh:'滋補腎陰、強腰膝、改善下肢冰冷與足踝僵硬',vi:''},
    technique:{zh:'用大拇指指腹輕柔揉按，力度由輕漸重',vi:''},
    caution:{zh:'局部若有急性扭傷、紅腫熱痛時應先冰敷休息，不宜直接大力推拿',vi:''}},
  {id:'leg_4',bodyPart:'leg',
    name:{zh:'三陰交穴',vi:''},
    meridian:{zh:'足太陰脾經',vi:''},
    keywords:{zh:['調節循環','健脾利濕','舒緩小腿內側肌肉疲勞','三陰交穴','三陰交'],vi:[]},
    location:{zh:'位於小腿內側，內踝尖直上四橫指（自內踝尖上三寸），脛骨後緣凹陷處',vi:''},
    benefit:{zh:'調節循環、健脾利濕、舒緩小腿內側肌肉疲勞',vi:''},
    technique:{zh:'以大拇指指腹或食指指腹按揉，配合緩慢呼吸施力',vi:''},
    caution:{zh:'孕婦禁按（此穴位通氣血，傳統上為安胎禁忌穴位）',vi:''}},
  {id:'leg_5',bodyPart:'leg',
    name:{zh:'足三里穴',vi:''},
    meridian:{zh:'足陽明胃經',vi:''},
    keywords:{zh:['調理脾胃','補氣血','消除下肢沉重與緩解疲勞','足三里穴','足三里'],vi:[]},
    location:{zh:'位於小腿外側，外膝眼直下四橫指（約三寸），脛骨前緣外側一橫指處',vi:''},
    benefit:{zh:'調理脾胃、補氣血、消除下肢沉重與緩解疲勞',vi:''},
    technique:{zh:'以拇指指腹或掌根按壓、順時針揉動，或用敲擊方式放鬆',vi:''},
    caution:{zh:'皮膚有濕疹、靜脈曲張嚴重或局部感染處應避開',vi:''}},
  {id:'leg_6',bodyPart:'leg',
    name:{zh:'承山穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['強筋健骨','放鬆小腿後側肌肉','緩解腓腸肌痙攣（抽筋）','承山穴','承山'],vi:[]},
    location:{zh:'位於小腿後側正中，當伸直小腿或足跟上提時，腓腸肌肌肉腹部下端之交角凹陷處（約小腿後側正中線）',vi:''},
    benefit:{zh:'強筋健骨、放鬆小腿後側肌肉、緩解腓腸肌痙攣（抽筋）',vi:''},
    technique:{zh:'以大拇指或掌根由下往上推揉，或適度握拳輕捶',vi:''},
    caution:{zh:'力度需依個人耐受度調整，若肌肉處於嚴重撕裂傷或急性拉傷期應避免強力揉捏',vi:''}},
  {id:'leg_7',bodyPart:'leg',
    name:{zh:'行間穴',vi:''},
    meridian:{zh:'足厥陰肝經',vi:''},
    keywords:{zh:['清肝瀉火','涼血睦目','舒緩足部悶熱與情緒亢奮','行間穴','行間'],vi:[]},
    location:{zh:'位於足背側，第一、二趾縫紋頭端的赤白肉際處',vi:''},
    benefit:{zh:'清肝瀉火、涼血睦目、舒緩足部悶熱與情緒亢奮',vi:''},
    technique:{zh:'用大拇指指腹點按並前後推揉',vi:''},
    caution:{zh:'局部有皮膚破損、感染或香港腳潰爛處請避開',vi:''}},
  {id:'leg_8',bodyPart:'leg',
    name:{zh:'大敦穴',vi:''},
    meridian:{zh:'足厥陰肝經',vi:''},
    keywords:{zh:['調理下焦','疏肝理氣','改善足趾冰冷','大敦穴','大敦'],vi:[]},
    location:{zh:'位於足大趾末節外側，趾甲根部旁開約一分（約0.1寸）',vi:''},
    benefit:{zh:'調理下焦、疏肝理氣、改善足趾冰冷',vi:''},
    technique:{zh:'用指甲輕掐或以指腹輕柔捏按',vi:''},
    caution:{zh:'孕婦及經期女性若要刺激需手法輕柔，避免過度強烈刺激',vi:''}},
  {id:'leg_9',bodyPart:'leg',
    name:{zh:'隱白穴',vi:''},
    meridian:{zh:'足太陰脾經',vi:''},
    keywords:{zh:['健脾和胃','統血調經','放鬆足趾末梢神經','隱白穴','隱白'],vi:[]},
    location:{zh:'位於足大趾末節內側，趾甲根部旁開約一分（約0.1寸）',vi:''},
    benefit:{zh:'健脾和胃、統血調經、放鬆足趾末梢神經',vi:''},
    technique:{zh:'以拇指與食指指腹輕捏並點按趾端',vi:''},
    caution:{zh:'局部皮膚有外傷或感染發炎時暫停按摩',vi:''}},
  {id:'leg_10',bodyPart:'leg',
    name:{zh:'然谷穴',vi:''},
    meridian:{zh:'足少陰腎經',vi:''},
    keywords:{zh:['清熱利濕','滋陰降火','舒緩足弓內側緊繃感','然谷穴','然谷'],vi:[]},
    location:{zh:'位於足內側，足舟骨粗隆下方凹陷處（赤白肉際交界處）',vi:''},
    benefit:{zh:'清熱利濕、滋陰降火、舒緩足弓內側緊繃感',vi:''},
    technique:{zh:'以大拇指指腹由下往上輕推按揉',vi:''},
    caution:{zh:'孕婦若有習慣性流產傾向者不宜重按',vi:''}},
  {id:'leg_11',bodyPart:'leg',
    name:{zh:'公孫穴',vi:''},
    meridian:{zh:'足太陰脾經',vi:''},
    keywords:{zh:['健脾胃','理氣機','舒緩胃部不適與足內側痠脹','公孫穴','公孫'],vi:[]},
    location:{zh:'位於足內側緣，第一蹠骨基底的前下方凹陷處（赤白肉際處）',vi:''},
    benefit:{zh:'健脾胃、理氣機、舒緩胃部不適與足內側痠脹',vi:''},
    technique:{zh:'以大拇指腹端順著骨緣前後緩慢推揉',vi:''},
    caution:{zh:'局部有骨骼壓痛或嚴重軟組織發炎時，手法應輕柔和緩',vi:''}},
  {id:'leg_12',bodyPart:'leg',
    name:{zh:'照海穴',vi:''},
    meridian:{zh:'足少陰腎經',vi:''},
    keywords:{zh:['滋腎清熱','寧神調經','緩解足踝內側腫脹與緊繃','照海穴','照海'],vi:[]},
    location:{zh:'位於足內側，內踝尖下方凹陷處（內踝下約一寸）',vi:''},
    benefit:{zh:'滋腎清熱、寧神調經、緩解足踝內側腫脹與緊繃',vi:''},
    technique:{zh:'以大拇指指腹輕輕按揉內踝下方凹陷',vi:''},
    caution:{zh:'局部若有急性扭傷微血管破裂期應避免用力揉捏',vi:''}},
  {id:'leg_13',bodyPart:'leg',
    name:{zh:'金門穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['安神定驚','通經活絡','舒緩足外側抽痛','金門穴','金門'],vi:[]},
    location:{zh:'位於足外側部，足舟骨粗隆下緣，骰骨下方的凹陷處',vi:''},
    benefit:{zh:'安神定驚、通經活絡、舒緩足外側抽痛',vi:''},
    technique:{zh:'以指腹點按或輕柔打圈揉動',vi:''},
    caution:{zh:'足底外側有急性拉傷或痛風發作期不宜強力按壓',vi:''}},
  {id:'leg_14',bodyPart:'leg',
    name:{zh:'京骨穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['清熱明目','舒筋活絡','緩解足外側緣疲勞','京骨穴','京骨'],vi:[]},
    location:{zh:'位於足外側部，第五蹠骨粗隆下方，赤白肉際處',vi:''},
    benefit:{zh:'清熱明目、舒筋活絡、緩解足外側緣疲勞',vi:''},
    technique:{zh:'以大拇指指腹順著骨緣上下推按',vi:''},
    caution:{zh:'皮膚若有繭過厚或龜裂疼痛時應避免直接用力施壓',vi:''}},
  {id:'leg_15',bodyPart:'leg',
    name:{zh:'束骨穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['通經活絡','散風清熱','舒緩足小趾及外側僵硬','束骨穴','束骨'],vi:[]},
    location:{zh:'位於足外側，足小趾本節（第五蹠趾關節）的後方赤白肉際處',vi:''},
    benefit:{zh:'通經活絡、散風清熱、舒緩足小趾及外側僵硬',vi:''},
    technique:{zh:'用大拇指指腹點按並做小幅度揉動',vi:''},
    caution:{zh:'孕婦宜輕柔按摩，避免強烈刺激反射區',vi:''}},
  {id:'leg_16',bodyPart:'leg',
    name:{zh:'至陰穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['通經理氣','調整氣血','放鬆足趾緊繃感','至陰穴','至陰'],vi:[]},
    location:{zh:'位於足小趾末節外側，趾甲根部旁開約一分（約0.1寸）',vi:''},
    benefit:{zh:'通經理氣、調整氣血、放鬆足趾緊繃感',vi:''},
    technique:{zh:'以指甲輕掐或指腹輕揉小趾外側',vi:''},
    caution:{zh:'孕婦慎用（傳統上此穴多用於轉胎，孕期日常足部按摩若無專業指引請避開此趾末端）',vi:''}},
  {id:'leg_17',bodyPart:'leg',
    name:{zh:'足臨泣',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['疏肝膽','清頭目','舒緩足背外側及腳趾縫間緊繃','足臨泣'],vi:[]},
    location:{zh:'位於足背側，第四、五蹠骨結合部的前方凹陷處',vi:''},
    benefit:{zh:'疏肝膽、清頭目、舒緩足背外側及腳趾縫間緊繃',vi:''},
    technique:{zh:'以指腹在兩趾骨之間縫隙處按壓揉動',vi:''},
    caution:{zh:'孕婦不宜過度強力刺激，以免引起氣血運行過快',vi:''}},
  {id:'leg_18',bodyPart:'leg',
    name:{zh:'丘墟穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['疏肝利膽','通絡止痛','舒緩足踝關節緊繃與外側痠痛','丘墟穴','丘墟'],vi:[]},
    location:{zh:'位於足外側，外踝前下方，趾長伸肌腱的外側凹陷處',vi:''},
    benefit:{zh:'疏肝利膽、通絡止痛、舒緩足踝關節緊繃與外側痠痛',vi:''},
    technique:{zh:'用大拇指指腹在外踝前下方畫圈揉按',vi:''},
    caution:{zh:'足踝部若有急性韌帶撕裂或嚴重扭傷腫脹時嚴禁按壓',vi:''}},
  {id:'leg_19',bodyPart:'leg',
    name:{zh:'懸鐘穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['疏風清熱','利膽通絡','舒緩小腿外側腓骨長短肌緊繃','懸鐘穴','懸鐘'],vi:[]},
    location:{zh:'位於小腿外側，外踝尖直上三橫指（約三寸），腓骨前緣凹陷處',vi:''},
    benefit:{zh:'疏風清熱、利膽通絡、舒緩小腿外側腓骨長短肌緊繃',vi:''},
    technique:{zh:'以大拇指指腹強力點揉，或由下往上推按',vi:''},
    caution:{zh:'孕婦若無特殊需求，小腿外側強刺激穴位宜力道適中',vi:''}},
  {id:'leg_20',bodyPart:'leg',
    name:{zh:'光明穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['清肝明目','通經活絡','緩解小腿外側痠脹沉重','光明穴','光明'],vi:[]},
    location:{zh:'位於小腿外側，外踝尖直上五橫指（約五寸），腓骨前緣凹陷處',vi:''},
    benefit:{zh:'清肝明目、通經活絡、緩解小腿外側痠脹沉重',vi:''},
    technique:{zh:'以大拇指或食指指腹強力點揉或上下推按',vi:''},
    caution:{zh:'局部皮膚若有靜脈曲張突起或皮膚病變應避開',vi:''}},
  {id:'leg_21',bodyPart:'leg',
    name:{zh:'陽輔穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['清熱膽經','通經止痛','舒緩小腿外側後側肌肉筋膜','陽輔穴','陽輔'],vi:[]},
    location:{zh:'位於小腿外側，外踝尖直上七橫指（約四寸），腓骨後緣凹陷處',vi:''},
    benefit:{zh:'清熱膽經、通經止痛、舒緩小腿外側後側肌肉筋膜',vi:''},
    technique:{zh:'以指腹按揉或用掌根輕輕推壓外側肌肉',vi:''},
    caution:{zh:'局部若有肌肉急性拉傷或骨膜發炎應避免重壓',vi:''}},
  {id:'leg_22',bodyPart:'leg',
    name:{zh:'築賓穴',vi:''},
    meridian:{zh:'足少陰腎經',vi:''},
    keywords:{zh:['清熱化濕','寧心安神','緩解小腿內側深層肌肉緊繃','築賓穴','築賓'],vi:[]},
    location:{zh:'位於小腿內側，內踝尖直上約五寸（太溪穴與陰谷穴連線上），腓腸肌肌腹的內下方',vi:''},
    benefit:{zh:'清熱化濕、寧心安神、緩解小腿內側深層肌肉緊繃',vi:''},
    technique:{zh:'以拇指指腹適度施力按揉',vi:''},
    caution:{zh:'孕婦不宜重按，以免造成下腹部不適',vi:''}},
  {id:'leg_23',bodyPart:'leg',
    name:{zh:'交信穴',vi:''},
    meridian:{zh:'足少陰腎經',vi:''},
    keywords:{zh:['調理氣血','利濕健脾','舒緩小腿內側脛骨後肌群緊繃','交信穴','交信'],vi:[]},
    location:{zh:'位於小腿內側，內踝尖直上二橫指（約二寸），脛骨內側面後緣',vi:''},
    benefit:{zh:'調理氣血、利濕健脾、舒緩小腿內側脛骨後肌群緊繃',vi:''},
    technique:{zh:'以指腹輕輕揉按脛骨後緣凹陷處',vi:''},
    caution:{zh:'懷孕期間按摩應力道溫和，避開強烈強刺激',vi:''}},
  {id:'leg_24',bodyPart:'leg',
    name:{zh:'復溜穴',vi:''},
    meridian:{zh:'足少陰腎經',vi:''},
    keywords:{zh:['補腎益陰','濕潤肌膚','改善下肢水腫與循環不良','復溜穴','復溜'],vi:[]},
    location:{zh:'位於小腿內側，太溪穴直上二橫指（約二寸），跟腱的前方',vi:''},
    benefit:{zh:'補腎益陰、濕潤肌膚、改善下肢水腫與循環不良',vi:''},
    technique:{zh:'以大拇指指腹輕柔按揉或上下推移',vi:''},
    caution:{zh:'局部若有靜脈曲張嚴重或皮膚紅腫發炎處請輕柔帶過',vi:''}},
  {id:'leg_25',bodyPart:'leg',
    name:{zh:'承筋穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['強筋健骨','放鬆小腿後側大塊肌肉','緩解緊繃與抽筋','承筋穴','承筋'],vi:[]},
    location:{zh:'位於小腿後側正中，委中穴與承山穴連線的中點，腓腸肌肌腹中央',vi:''},
    benefit:{zh:'強筋健骨、放鬆小腿後側大塊肌肉、緩解緊繃與抽筋',vi:''},
    technique:{zh:'以掌根或大拇指由下往上推揉小腿肚核心',vi:''},
    caution:{zh:'若小腿肌肉正處於嚴重撕裂傷或劇烈抽筋剛過時，應由輕而重，切忌猛力粗暴揉捏',vi:''}},
  {id:'shoulder_1',bodyPart:'shoulder',
    name:{zh:'肩井穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['疏散風熱','通經活絡','緩解肩頸僵硬與落枕','肩井穴','肩井'],vi:[]},
    location:{zh:'位於肩上，大椎穴與肩峰端連線的中點，肩部筋肉處',vi:''},
    benefit:{zh:'疏散風熱、通經活絡、緩解肩頸僵硬與落枕',vi:''},
    technique:{zh:'以大拇指或中指指腹垂直向下按壓，或用掌根輕揉',vi:''},
    caution:{zh:'孕婦禁按（此穴位屬強刺激穴，易引起強烈宮縮）',vi:''}},
  {id:'shoulder_2',bodyPart:'shoulder',
    name:{zh:'肩髃穴',vi:''},
    meridian:{zh:'手陽明大腸經',vi:''},
    keywords:{zh:['利關節','通經絡','緩解肩關節疼痛與上肢活動不利','肩髃穴','肩髃'],vi:[]},
    location:{zh:'位於肩部，當肩峰端下緣，肩峰與肱骨大結節之間，三角肌上部凹陷處',vi:''},
    benefit:{zh:'利關節、通經絡、緩解肩關節疼痛與上肢活動不利',vi:''},
    technique:{zh:'用大拇指指腹點按，並配合肩部輕微前後轉動',vi:''},
    caution:{zh:'局部若有急性發炎、紅腫熱痛應避免強力按壓',vi:''}},
  {id:'shoulder_3',bodyPart:'shoulder',
    name:{zh:'肩髎穴',vi:''},
    meridian:{zh:'手少陽三焦經',vi:''},
    keywords:{zh:['通經活絡','利關節','緩解肩背痠痛與手臂麻木','肩髎穴','肩髎'],vi:[]},
    location:{zh:'位於肩部，肩髃穴後方，當臂外展時，肩峰後下方凹陷處',vi:''},
    benefit:{zh:'通經活絡、利關節、緩解肩背痠痛與手臂麻木',vi:''},
    technique:{zh:'以指腹點按或輕柔打圈揉動',vi:''},
    caution:{zh:'避免局部肌肉拉傷急性期用力揉捏',vi:''}},
  {id:'shoulder_4',bodyPart:'shoulder',
    name:{zh:'肩貞穴',vi:''},
    meridian:{zh:'手太陽小腸經',vi:''},
    keywords:{zh:['通經活絡','散風解表','緩解肩胛痠痛與手臂無力','肩貞穴','肩貞'],vi:[]},
    location:{zh:'位於肩關節後下方，當臂內收時，腋後紋頭上1寸處',vi:''},
    benefit:{zh:'通經活絡、散風解表、緩解肩胛痠痛與手臂無力',vi:''},
    technique:{zh:'以指腹按揉，力道由輕漸重',vi:''},
    caution:{zh:'局部若有皮膚病變或淋巴結腫大應避開',vi:''}},
  {id:'shoulder_5',bodyPart:'shoulder',
    name:{zh:'曲垣穴',vi:''},
    meridian:{zh:'手太陽小腸經',vi:''},
    keywords:{zh:['舒筋活絡','利肩胛','緩解肩背深層肌肉痠脹','曲垣穴','曲垣'],vi:[]},
    location:{zh:'位於肩胛骨岡上窩內側端（在肩胛岡上方凹陷處）',vi:''},
    benefit:{zh:'舒筋活絡、利肩胛、緩解肩背深層肌肉痠脹',vi:''},
    technique:{zh:'以大拇指腹由上往下推按或深層點按',vi:''},
    caution:{zh:'應避免施力過猛造成軟組織挫傷',vi:''}},
  {id:'shoulder_6',bodyPart:'shoulder',
    name:{zh:'天宗穴',vi:''},
    meridian:{zh:'手太陽小腸經',vi:''},
    keywords:{zh:['舒筋活絡','理氣消腫','緩解肩胛部疼痛與胸悶','天宗穴','天宗'],vi:[]},
    location:{zh:'位於肩胛骨岡下窩中央凹陷處（約與第四胸椎平齊）',vi:''},
    benefit:{zh:'舒筋活絡、理氣消腫、緩解肩胛部疼痛與胸悶',vi:''},
    technique:{zh:'以大拇指指腹深層點壓，配合肩部輕緩旋轉',vi:''},
    caution:{zh:'若伴隨有胸部不適或急性心悸，按摩前應先行評估',vi:''}},
  {id:'shoulder_7',bodyPart:'shoulder',
    name:{zh:'秉風穴',vi:''},
    meridian:{zh:'手太陽小腸經',vi:''},
    keywords:{zh:['散風活絡','利肩胛','緩解肩背部痠痛與活動受限','秉風穴','秉風'],vi:[]},
    location:{zh:'位於肩胛骨岡上窩中，天宗穴直上，舉臂時有凹陷處',vi:''},
    benefit:{zh:'散風活絡、利肩胛、緩解肩背部痠痛與活動受限',vi:''},
    technique:{zh:'以指腹點按，配合肩部舒展動作',vi:''},
    caution:{zh:'避免過度強力按壓以免引起肩部韌帶不適',vi:''}},
  {id:'shoulder_8',bodyPart:'shoulder',
    name:{zh:'臑俞穴',vi:''},
    meridian:{zh:'手太陽小腸經',vi:''},
    keywords:{zh:['通經活絡','利肩關節','緩解肩部痠痛與上肢活動受限','臑俞穴','臑俞'],vi:[]},
    location:{zh:'位於肩部，當腋後紋頭直上，肩胛岡下緣凹陷處',vi:''},
    benefit:{zh:'通經活絡、利肩關節、緩解肩部痠痛與上肢活動受限',vi:''},
    technique:{zh:'以指腹輕柔按揉',vi:''},
    caution:{zh:'避免局部骨膜炎症狀時大力施壓',vi:''}},
  {id:'shoulder_9',bodyPart:'shoulder',
    name:{zh:'巨骨穴',vi:''},
    meridian:{zh:'手陽明大腸經',vi:''},
    keywords:{zh:['通經活絡','利關節','緩解肩部與鎖骨區域痠痛','巨骨穴','巨骨'],vi:[]},
    location:{zh:'位於鎖骨肩峰端與肩胛岡之間凹陷處',vi:''},
    benefit:{zh:'通經活絡、利關節、緩解肩部與鎖骨區域痠痛',vi:''},
    technique:{zh:'以指腹輕緩點按',vi:''},
    caution:{zh:'此處靠近胸廓，按壓力度宜輕，避免過度壓迫',vi:''}},
  {id:'shoulder_10',bodyPart:'shoulder',
    name:{zh:'缺盆穴',vi:''},
    meridian:{zh:'足陽明胃經',vi:''},
    keywords:{zh:['寬胸利膈','通經活絡','緩解肩頸部氣血不暢','缺盆穴','缺盆'],vi:[]},
    location:{zh:'位於鎖骨上窩中央凹陷處',vi:''},
    benefit:{zh:'寬胸利膈、通經活絡、緩解肩頸部氣血不暢',vi:''},
    technique:{zh:'輕柔點按',vi:''},
    caution:{zh:'此處靠近頸動脈，嚴禁粗暴重壓，僅適度輕觸或輕揉',vi:''}},
  {id:'shoulder_11',bodyPart:'shoulder',
    name:{zh:'天鼎穴',vi:''},
    meridian:{zh:'手陽明大腸經',vi:''},
    keywords:{zh:['清利咽喉','通經活絡','舒緩肩頸緊繃與頸部活動受限','天鼎穴','天鼎'],vi:[]},
    location:{zh:'位於頸部側面，胸鎖乳突肌後緣，扶突穴直下一寸處',vi:''},
    benefit:{zh:'清利咽喉、通經活絡、舒緩肩頸緊繃與頸部活動受限',vi:''},
    technique:{zh:'以指腹輕揉按壓',vi:''},
    caution:{zh:'避免過度壓迫頸動脈',vi:''}},
  {id:'shoulder_12',bodyPart:'shoulder',
    name:{zh:'扶突穴',vi:''},
    meridian:{zh:'手陽明大腸經',vi:''},
    keywords:{zh:['理氣降逆','清利咽喉','改善頸部循環','扶突穴','扶突'],vi:[]},
    location:{zh:'位於頸部側面，喉結旁開三寸，胸鎖乳突肌胸骨頭與鎖骨頭之間',vi:''},
    benefit:{zh:'理氣降逆、清利咽喉、改善頸部循環',vi:''},
    technique:{zh:'以指腹輕輕按揉',vi:''},
    caution:{zh:'此處結構敏感，嚴禁重力壓迫',vi:''}},
  {id:'shoulder_13',bodyPart:'shoulder',
    name:{zh:'大椎穴',vi:''},
    meridian:{zh:'督脈',vi:''},
    keywords:{zh:['解表清熱','疏通經絡','緩解頸部僵硬與頭痛','大椎穴','大椎'],vi:[]},
    location:{zh:'位於第7頸椎棘突下凹陷處（低頭時最凸出的骨頭下方）',vi:''},
    benefit:{zh:'解表清熱、疏通經絡、緩解頸部僵硬與頭痛',vi:''},
    technique:{zh:'以掌根或大拇指腹輕揉，配合頸部輕緩活動',vi:''},
    caution:{zh:'避免骨骼疏鬆或頸椎損傷者大力按壓',vi:''}},
  {id:'shoulder_14',bodyPart:'shoulder',
    name:{zh:'肩中俞穴',vi:''},
    meridian:{zh:'手太陽小腸經',vi:''},
    keywords:{zh:['舒筋活絡','聰耳明目','緩解肩背部痠痛與頸部僵硬','肩中俞穴','肩中俞'],vi:[]},
    location:{zh:'位於背部，第7頸椎棘突下旁開二寸',vi:''},
    benefit:{zh:'舒筋活絡、聰耳明目、緩解肩背部痠痛與頸部僵硬',vi:''},
    technique:{zh:'以指腹點按揉動',vi:''},
    caution:{zh:'避免因局部脊椎炎症大力推按',vi:''}},
  {id:'shoulder_15',bodyPart:'shoulder',
    name:{zh:'肩外俞穴',vi:''},
    meridian:{zh:'手太陽小腸經',vi:''},
    keywords:{zh:['舒筋活絡','利肩胛','緩解肩背痠痛與上肢活動障礙','肩外俞穴','肩外俞'],vi:[]},
    location:{zh:'位於背部，第1胸椎棘突下旁開三寸',vi:''},
    benefit:{zh:'舒筋活絡、利肩胛、緩解肩背痠痛與上肢活動障礙',vi:''},
    technique:{zh:'以指腹點按揉動',vi:''},
    caution:{zh:'按摩時力道應和緩',vi:''}},
  {id:'shoulder_16',bodyPart:'shoulder',
    name:{zh:'天柱穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['清頭明目','舒緩頸部緊繃與頭痛','天柱穴','天柱'],vi:[]},
    location:{zh:'位於後頸部，後髮際正中直上一寸，旁開一寸三分（斜方肌外緣凹陷處）',vi:''},
    benefit:{zh:'清頭明目、舒緩頸部緊繃與頭痛',vi:''},
    technique:{zh:'以大拇指指腹向上托按',vi:''},
    caution:{zh:'此處連接腦部，按摩手法應柔和，避免直接重擊',vi:''}},
  {id:'shoulder_17',bodyPart:'shoulder',
    name:{zh:'風池穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['平肝熄風','清頭明目','緩解頸部痠痛與壓力性頭痛','風池穴','風池'],vi:[]},
    location:{zh:'位於後頸部，枕骨之下，胸鎖乳突肌與斜方肌上端之間的凹陷處',vi:''},
    benefit:{zh:'平肝熄風、清頭明目、緩解頸部痠痛與壓力性頭痛',vi:''},
    technique:{zh:'以雙手大拇指向上托揉',vi:''},
    caution:{zh:'避免力度過大，以免造成頭暈或不適',vi:''}},
  {id:'shoulder_18',bodyPart:'shoulder',
    name:{zh:'完骨穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['疏風清熱','安神定驚','緩解頸部側方緊繃','完骨穴','完骨'],vi:[]},
    location:{zh:'位於頭部側後方，乳突後下方凹陷處',vi:''},
    benefit:{zh:'疏風清熱、安神定驚、緩解頸部側方緊繃',vi:''},
    technique:{zh:'以指腹輕柔揉按',vi:''},
    caution:{zh:'避免重力按壓引起頭部不適',vi:''}},
  {id:'shoulder_19',bodyPart:'shoulder',
    name:{zh:'天牖穴',vi:''},
    meridian:{zh:'手少陽三焦經',vi:''},
    keywords:{zh:['清頭明目','通經活絡','舒緩頸部側面痠痛','天牖穴','天牖'],vi:[]},
    location:{zh:'位於頸側，乳突後下方，胸鎖乳突肌後緣凹陷處',vi:''},
    benefit:{zh:'清頭明目、通經活絡、舒緩頸部側面痠痛',vi:''},
    technique:{zh:'以指腹輕按揉動',vi:''},
    caution:{zh:'手法需輕柔，避開血管神經豐富區域',vi:''}},
  {id:'shoulder_20',bodyPart:'shoulder',
    name:{zh:'頸百勞穴',vi:''},
    meridian:{zh:'經外奇穴',vi:''},
    keywords:{zh:['止咳平喘','舒緩頸項強痛','頸百勞穴','頸百勞'],vi:[]},
    location:{zh:'位於項部，大椎穴直上一寸，旁開一寸',vi:''},
    benefit:{zh:'止咳平喘、舒緩頸項強痛',vi:''},
    technique:{zh:'以指腹點按或輕柔打圈揉按',vi:''},
    caution:{zh:'力度需適中，避免引起頸椎不適',vi:''}},
  {id:'upperback_1',bodyPart:'upperback',
    name:{zh:'大杼穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['疏風解表','強筋健骨','緩解背部僵硬與痠痛','大杼穴','大杼'],vi:[]},
    location:{zh:'位於背部，第1胸椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'疏風解表、強筋健骨、緩解背部僵硬與痠痛',vi:''},
    technique:{zh:'以大拇指或掌根由內向外輕柔推按',vi:''},
    caution:{zh:'局部有骨質疏鬆或脊椎損傷時應極度小心，力道宜輕',vi:''}},
  {id:'upperback_2',bodyPart:'upperback',
    name:{zh:'風門穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['祛風散寒','宣肺解表','緩解感冒引起的背部緊繃','風門穴','風門'],vi:[]},
    location:{zh:'位於背部，第2胸椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'祛風散寒、宣肺解表、緩解感冒引起的背部緊繃',vi:''},
    technique:{zh:'以大拇指腹點按或輕柔打圈揉動',vi:''},
    caution:{zh:'按摩後應注意保暖，避免吹風受涼',vi:''}},
  {id:'upperback_3',bodyPart:'upperback',
    name:{zh:'肺俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['理氣降逆','宣肺止咳','緩解胸悶與背部緊繃','肺俞穴','肺俞'],vi:[]},
    location:{zh:'位於背部，第3胸椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'理氣降逆、宣肺止咳、緩解胸悶與背部緊繃',vi:''},
    technique:{zh:'以掌根或大拇指輕柔揉按',vi:''},
    caution:{zh:'避免過度深層壓迫，按壓時配合緩慢呼吸',vi:''}},
  {id:'upperback_4',bodyPart:'upperback',
    name:{zh:'厥陰俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['寬胸理氣','寧心安神','緩解心悸與背部痠脹','厥陰俞穴','厥陰俞'],vi:[]},
    location:{zh:'位於背部，第4胸椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'寬胸理氣、寧心安神、緩解心悸與背部痠脹',vi:''},
    technique:{zh:'以指腹輕柔點按',vi:''},
    caution:{zh:'此處靠近心臟對應區，按壓手法需極為細膩，不可重力衝擊',vi:''}},
  {id:'upperback_5',bodyPart:'upperback',
    name:{zh:'心俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['寧心安神','理氣止痛','緩解胸悶與背部壓力性緊繃','心俞穴','心俞'],vi:[]},
    location:{zh:'位於背部，第5胸椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'寧心安神、理氣止痛、緩解胸悶與背部壓力性緊繃',vi:''},
    technique:{zh:'以指腹輕緩按壓，配合舒緩呼吸',vi:''},
    caution:{zh:'心臟病患者按摩此區需特別謹慎，建議避開重壓',vi:''}},
  {id:'upperback_6',bodyPart:'upperback',
    name:{zh:'督俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['理氣止痛','寧心安神','緩解胸脅區域痠痛','督俞穴','督俞'],vi:[]},
    location:{zh:'位於背部，第6胸椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'理氣止痛、寧心安神、緩解胸脅區域痠痛',vi:''},
    technique:{zh:'以指腹輕柔按揉',vi:''},
    caution:{zh:'避免局部軟組織拉傷急性期用力',vi:''}},
  {id:'upperback_7',bodyPart:'upperback',
    name:{zh:'膈俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['寬胸順氣','活血化瘀','緩解橫膈區域僵硬與背痛','膈俞穴','膈俞'],vi:[]},
    location:{zh:'位於背部，第7胸椎棘突下，旁開一寸半（為八會穴之血會）',vi:''},
    benefit:{zh:'寬胸順氣、活血化瘀、緩解橫膈區域僵硬與背痛',vi:''},
    technique:{zh:'以大拇指腹點按揉動',vi:''},
    caution:{zh:'力道應溫和，不宜過度粗暴揉捏',vi:''}},
  {id:'upperback_8',bodyPart:'upperback',
    name:{zh:'定喘穴',vi:''},
    meridian:{zh:'經外奇穴',vi:''},
    keywords:{zh:['止咳平喘','緩解背部肩胛間區僵硬','定喘穴','定喘'],vi:[]},
    location:{zh:'位於背部，第7頸椎棘突下旁開0.5寸處',vi:''},
    benefit:{zh:'止咳平喘、緩解背部肩胛間區僵硬',vi:''},
    technique:{zh:'以指腹輕點或輕揉',vi:''},
    caution:{zh:'手法要輕巧，避免壓迫脊椎骨節',vi:''}},
  {id:'upperback_9',bodyPart:'upperback',
    name:{zh:'夾脊穴（胸部段）',vi:''},
    meridian:{zh:'經外奇穴',vi:''},
    keywords:{zh:['調節臟腑','通經活絡','改善全背部肌肉痠痛','夾脊穴（胸部段）'],vi:[]},
    location:{zh:'位於背部，第1至第12胸椎棘突下旁開0.5寸（一串穴位）',vi:''},
    benefit:{zh:'調節臟腑、通經活絡、改善全背部肌肉痠痛',vi:''},
    technique:{zh:'以拇指沿脊椎旁開兩側緩慢滑推',vi:''},
    caution:{zh:'不可直接重力按壓脊椎骨，需在肌肉軟組織上施力',vi:''}},
  {id:'upperback_10',bodyPart:'upperback',
    name:{zh:'巨闕俞',vi:''},
    meridian:{zh:'足太陽膀胱經（相關區域）',vi:''},
    keywords:{zh:['理氣寬中','緩解背部深層痠痛','巨闕俞'],vi:[]},
    location:{zh:'位於背部，第6胸椎棘突下，旁開三寸',vi:''},
    benefit:{zh:'理氣寬中、緩解背部深層痠痛',vi:''},
    technique:{zh:'以指腹點按',vi:''},
    caution:{zh:'避免局部肌肉急性挫傷處用力',vi:''}},
  {id:'upperback_11',bodyPart:'upperback',
    name:{zh:'神堂穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['寬胸寧心','理氣止痛','舒緩背部兩側緊繃','神堂穴','神堂'],vi:[]},
    location:{zh:'位於背部，第5胸椎棘突下，旁開三寸',vi:''},
    benefit:{zh:'寬胸寧心、理氣止痛、舒緩背部兩側緊繃',vi:''},
    technique:{zh:'以指腹輕揉或掌根按壓',vi:''},
    caution:{zh:'手法需穩定和緩，避免造成肋骨不適',vi:''}},
  {id:'upperback_12',bodyPart:'upperback',
    name:{zh:'譩譆穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['寬胸理氣','舒緩背部僵硬','譩譆穴','譩譆'],vi:[]},
    location:{zh:'位於背部，第6胸椎棘突下，旁開三寸',vi:''},
    benefit:{zh:'寬胸理氣、舒緩背部僵硬',vi:''},
    technique:{zh:'以指腹點按揉動',vi:''},
    caution:{zh:'按壓時需注意背部肌群耐受度',vi:''}},
  {id:'upperback_13',bodyPart:'upperback',
    name:{zh:'膈關穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['寬胸降逆','健脾和胃','緩解背部不適','膈關穴','膈關'],vi:[]},
    location:{zh:'位於背部，第7胸椎棘突下，旁開三寸',vi:''},
    benefit:{zh:'寬胸降逆、健脾和胃、緩解背部不適',vi:''},
    technique:{zh:'以大拇指或掌根輕輕揉按',vi:''},
    caution:{zh:'避免粗暴動作，配合呼吸施力',vi:''}},
  {id:'upperback_14',bodyPart:'upperback',
    name:{zh:'魄戶穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['潤肺平喘','疏通背部氣血','魄戶穴','魄戶'],vi:[]},
    location:{zh:'位於背部，第3胸椎棘突下，旁開三寸',vi:''},
    benefit:{zh:'潤肺平喘、疏通背部氣血',vi:''},
    technique:{zh:'以指腹輕柔推按',vi:''},
    caution:{zh:'此處肌層較薄，按壓力道應輕',vi:''}},
  {id:'upperback_15',bodyPart:'upperback',
    name:{zh:'膏肓穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['補虛益損','調和氣血','緩解背部深層頑固僵硬','膏肓穴','膏肓'],vi:[]},
    location:{zh:'位於背部，第4胸椎棘突下，旁開三寸',vi:''},
    benefit:{zh:'補虛益損、調和氣血、緩解背部深層頑固僵硬',vi:''},
    technique:{zh:'以大拇指腹深層點按（常稱「病入膏肓」即指此處）',vi:''},
    caution:{zh:'此穴按壓感強烈，需適度溝通力道，避免造成皮下瘀血',vi:''}},
  {id:'upperback_16',bodyPart:'upperback',
    name:{zh:'神堂穴（相關區）',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['寧心安神','緩解背肌緊繃','神堂穴（相關區）'],vi:[]},
    location:{zh:'位於背部，第5胸椎棘突下，旁開三寸',vi:''},
    benefit:{zh:'寧心安神、緩解背肌緊繃',vi:''},
    technique:{zh:'以指腹輕柔揉按',vi:''},
    caution:{zh:'保持輕柔節奏',vi:''}},
  {id:'upperback_17',bodyPart:'upperback',
    name:{zh:'志室穴（上段延伸）',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['益腎強腰','舒緩背部痠痛','志室穴（上段延伸）'],vi:[]},
    location:{zh:'位於背部，第14胸椎棘突下，旁開三寸（雖偏中背，但常做為背部放鬆點）',vi:''},
    benefit:{zh:'益腎強腰、舒緩背部痠痛',vi:''},
    technique:{zh:'以掌根推揉',vi:''},
    caution:{zh:'手法應連貫柔和',vi:''}},
  {id:'upperback_18',bodyPart:'upperback',
    name:{zh:'脊中穴',vi:''},
    meridian:{zh:'督脈',vi:''},
    keywords:{zh:['健脾利濕','疏通督脈','緩解背部中心痠痛','脊中穴','脊中'],vi:[]},
    location:{zh:'位於背部，第11胸椎棘突下凹陷處',vi:''},
    benefit:{zh:'健脾利濕、疏通督脈、緩解背部中心痠痛',vi:''},
    technique:{zh:'以指腹輕揉',vi:''},
    caution:{zh:'脊椎骨突處不可大力重壓',vi:''}},
  {id:'upperback_19',bodyPart:'upperback',
    name:{zh:'中樞穴',vi:''},
    meridian:{zh:'督脈',vi:''},
    keywords:{zh:['健脾和胃','寬胸理氣','緩解背部緊繃','中樞穴','中樞'],vi:[]},
    location:{zh:'位於背部，第10胸椎棘突下凹陷處',vi:''},
    benefit:{zh:'健脾和胃、寬胸理氣、緩解背部緊繃',vi:''},
    technique:{zh:'以指腹或掌根輕揉',vi:''},
    caution:{zh:'力道需輕緩，不可用力衝擊',vi:''}},
  {id:'upperback_20',bodyPart:'upperback',
    name:{zh:'懸樞穴',vi:''},
    meridian:{zh:'督脈',vi:''},
    keywords:{zh:['健脾益氣','緩解腰背痠痛','懸樞穴','懸樞'],vi:[]},
    location:{zh:'位於背部，第13胸椎（即第1腰椎）棘突下凹陷處',vi:''},
    benefit:{zh:'健脾益氣、緩解腰背痠痛',vi:''},
    technique:{zh:'以指腹輕揉',vi:''},
    caution:{zh:'避免過度壓迫椎間盤位置',vi:''}},
  {id:'lowerback_1',bodyPart:'lowerback',
    name:{zh:'腎俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['補腎強腰','利水消腫','緩解腰部痠痛與疲勞','腎俞穴','腎俞'],vi:[]},
    location:{zh:'位於背部，第2腰椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'補腎強腰、利水消腫、緩解腰部痠痛與疲勞',vi:''},
    technique:{zh:'以大拇指或掌根由內向外按揉，或以握拳輕叩',vi:''},
    caution:{zh:'局部若有急性腰扭傷應輕柔為主，不宜強烈敲擊',vi:''}},
  {id:'lowerback_2',bodyPart:'lowerback',
    name:{zh:'氣海俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['理氣益氣','調和下焦','緩解腰腹痠脹','氣海俞穴','氣海俞'],vi:[]},
    location:{zh:'位於背部，第3腰椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'理氣益氣、調和下焦、緩解腰腹痠脹',vi:''},
    technique:{zh:'以大拇指腹按揉',vi:''},
    caution:{zh:'力道宜和緩，避免造成椎體過度受壓',vi:''}},
  {id:'lowerback_3',bodyPart:'lowerback',
    name:{zh:'大腸俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['通調腸胃','理氣強腰','緩解腰骶部疼痛與脹氣','大腸俞穴','大腸俞'],vi:[]},
    location:{zh:'位於背部，第4腰椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'通調腸胃、理氣強腰、緩解腰骶部疼痛與脹氣',vi:''},
    technique:{zh:'以掌根或大拇指按揉，配合腰部輕微左右擺動',vi:''},
    caution:{zh:'此處為腰部負荷區，按摩時應注意腰椎舒適度',vi:''}},
  {id:'lowerback_4',bodyPart:'lowerback',
    name:{zh:'關元俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['補腎固精','強腰利濕','緩解腰骶痠痛','關元俞穴','關元俞'],vi:[]},
    location:{zh:'位於背部，第5腰椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'補腎固精、強腰利濕、緩解腰骶痠痛',vi:''},
    technique:{zh:'以指腹按揉',vi:''},
    caution:{zh:'力道需均勻，避免對骨性突起處造成不適',vi:''}},
  {id:'lowerback_5',bodyPart:'lowerback',
    name:{zh:'小腸俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['疏通下焦','利濕熱','緩解骶部與下腹疼痛','小腸俞穴','小腸俞'],vi:[]},
    location:{zh:'位於背部，第1骶椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'疏通下焦、利濕熱、緩解骶部與下腹疼痛',vi:''},
    technique:{zh:'以指腹輕揉或掌根按壓',vi:''},
    caution:{zh:'手法宜輕巧，避開骶骨過硬區',vi:''}},
  {id:'lowerback_6',bodyPart:'lowerback',
    name:{zh:'膀胱俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['利尿通淋','強腰膝','緩解下腰部與骨盆區域痠痛','膀胱俞穴','膀胱俞'],vi:[]},
    location:{zh:'位於背部，第2骶椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'利尿通淋、強腰膝、緩解下腰部與骨盆區域痠痛',vi:''},
    technique:{zh:'以大拇指按壓，力道適中',vi:''},
    caution:{zh:'應避免對骶骨孔進行深層重壓',vi:''}},
  {id:'lowerback_7',bodyPart:'lowerback',
    name:{zh:'中膂俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['通經活絡','強腰脊','改善下背部僵硬','中膂俞穴','中膂俞'],vi:[]},
    location:{zh:'位於背部，第3骶椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'通經活絡、強腰脊、改善下背部僵硬',vi:''},
    technique:{zh:'以指腹點按',vi:''},
    caution:{zh:'避免局部神經豐富區域施力過重',vi:''}},
  {id:'lowerback_8',bodyPart:'lowerback',
    name:{zh:'白環俞穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['調經固帶','強腰脊','緩解腰骶與髖部緊繃','白環俞穴','白環俞'],vi:[]},
    location:{zh:'位於背部，第4骶椎棘突下，旁開一寸半',vi:''},
    benefit:{zh:'調經固帶、強腰脊、緩解腰骶與髖部緊繃',vi:''},
    technique:{zh:'以指腹輕按揉動',vi:''},
    caution:{zh:'手法需溫和，避開骨性突起邊緣',vi:''}},
  {id:'lowerback_9',bodyPart:'lowerback',
    name:{zh:'八髎穴（上、次、中、下髎）',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['調和氣血','強腰強身','改善下背部與生殖泌尿系統循環','八髎穴（上、次、中、下髎）'],vi:[]},
    location:{zh:'位於骶骨孔處（共四對八穴）',vi:''},
    benefit:{zh:'調和氣血、強腰強身、改善下背部與生殖泌尿系統循環',vi:''},
    technique:{zh:'以掌根在骶骨區域由上向下揉推，或以拇指精準點按骶孔',vi:''},
    caution:{zh:'此處骨骼淺，力道不可過猛，避免直接衝擊骨膜',vi:''}},
  {id:'lowerback_10',bodyPart:'lowerback',
    name:{zh:'志室穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['益腎強腰','舒緩腰部兩側肌肉僵硬','志室穴','志室'],vi:[]},
    location:{zh:'位於背部，第2腰椎棘突下，旁開三寸',vi:''},
    benefit:{zh:'益腎強腰、舒緩腰部兩側肌肉僵硬',vi:''},
    technique:{zh:'以大拇指按揉或掌根推摩',vi:''},
    caution:{zh:'若腰部有急性肌肉撕裂應避免強力動作',vi:''}},
  {id:'lowerback_11',bodyPart:'lowerback',
    name:{zh:'秩邊穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['舒筋活絡','強腰膝','緩解腰骶部至臀部的放射狀痠痛','秩邊穴','秩邊'],vi:[]},
    location:{zh:'位於臀部，第4骶椎棘突下，旁開三寸（近臀部梨狀肌處）',vi:''},
    benefit:{zh:'舒筋活絡、強腰膝、緩解腰骶部至臀部的放射狀痠痛',vi:''},
    technique:{zh:'以大拇指或肘尖深層點按（若非專業人員建議用掌根）',vi:''},
    caution:{zh:'若有坐骨神經急性發炎期應諮詢專業醫師，避免過度激惹神經',vi:''}},
  {id:'lowerback_12',bodyPart:'lowerback',
    name:{zh:'腰陽關穴',vi:''},
    meridian:{zh:'督脈',vi:''},
    keywords:{zh:['強腰膝','祛寒濕','緩解腰骶部僵硬與寒冷感','腰陽關穴','腰陽關'],vi:[]},
    location:{zh:'位於背部，第4腰椎棘突下凹陷處',vi:''},
    benefit:{zh:'強腰膝、祛寒濕、緩解腰骶部僵硬與寒冷感',vi:''},
    technique:{zh:'以指腹或掌根輕揉',vi:''},
    caution:{zh:'切忌暴力重按脊椎，以溫熱推按為主',vi:''}},
  {id:'lowerback_13',bodyPart:'lowerback',
    name:{zh:'命門穴',vi:''},
    meridian:{zh:'督脈',vi:''},
    keywords:{zh:['補腎壯陽','強腰益氣','緩解腰部痠軟無力','命門穴','命門'],vi:[]},
    location:{zh:'位於背部，第2腰椎棘突下凹陷處',vi:''},
    benefit:{zh:'補腎壯陽、強腰益氣、緩解腰部痠軟無力',vi:''},
    technique:{zh:'以大拇指輕柔揉按或掌根推按',vi:''},
    caution:{zh:'不可重壓，按摩時應配合保暖',vi:''}},
  {id:'lowerback_14',bodyPart:'lowerback',
    name:{zh:'懸樞穴',vi:''},
    meridian:{zh:'督脈',vi:''},
    keywords:{zh:['健脾益氣','強腰強脊','緩解腰背痠痛','懸樞穴','懸樞'],vi:[]},
    location:{zh:'位於背部，第1腰椎棘突下凹陷處',vi:''},
    benefit:{zh:'健脾益氣、強腰強脊、緩解腰背痠痛',vi:''},
    technique:{zh:'以指腹輕輕揉按',vi:''},
    caution:{zh:'避免造成椎間盤壓力',vi:''}},
  {id:'lowerback_15',bodyPart:'lowerback',
    name:{zh:'腰眼穴',vi:''},
    meridian:{zh:'經外奇穴',vi:''},
    keywords:{zh:['強腰健腎','緩解腰部肌肉慢性痠痛','腰眼穴','腰眼'],vi:[]},
    location:{zh:'位於腰部，第4腰椎棘突下旁開約3.5寸凹陷處（腰部「腰窩」位置）',vi:''},
    benefit:{zh:'強腰健腎、緩解腰部肌肉慢性痠痛',vi:''},
    technique:{zh:'以大拇指點按或掌根大面積揉搓',vi:''},
    caution:{zh:'手法需穩定，不可強力撞擊骨骼',vi:''}},
  {id:'lowerback_16',bodyPart:'lowerback',
    name:{zh:'痞根穴',vi:''},
    meridian:{zh:'經外奇穴',vi:''},
    keywords:{zh:['消積化痞','緩解腰背部深層緊繃','痞根穴','痞根'],vi:[]},
    location:{zh:'位於背部，第1腰椎棘突下旁開3.5寸',vi:''},
    benefit:{zh:'消積化痞、緩解腰背部深層緊繃',vi:''},
    technique:{zh:'以指腹輕按揉動',vi:''},
    caution:{zh:'按摩手法應輕柔，避免引起肋骨周邊不適',vi:''}},
  {id:'lowerback_17',bodyPart:'lowerback',
    name:{zh:'十七椎穴',vi:''},
    meridian:{zh:'經外奇穴',vi:''},
    keywords:{zh:['強腰膝','舒緩腰骶痠痛與痛經（相關反射）','十七椎穴','十七椎'],vi:[]},
    location:{zh:'位於背部，第5腰椎棘突下凹陷處',vi:''},
    benefit:{zh:'強腰膝、舒緩腰骶痠痛與痛經（相關反射）',vi:''},
    technique:{zh:'以大拇指或指腹輕揉',vi:''},
    caution:{zh:'力度要輕，避開骨骼突起處',vi:''}},
  {id:'lowerback_18',bodyPart:'lowerback',
    name:{zh:'下極俞穴',vi:''},
    meridian:{zh:'督脈（旁側延伸）',vi:''},
    keywords:{zh:['緩解腰部疲勞','下極俞穴','下極俞'],vi:[]},
    location:{zh:'位於背部，第3腰椎棘突下旁開處',vi:''},
    benefit:{zh:'緩解腰部疲勞',vi:''},
    technique:{zh:'以指腹輕柔按摩',vi:''},
    caution:{zh:'保持力道輕緩',vi:''}},
  {id:'lowerback_19',bodyPart:'lowerback',
    name:{zh:'大腸俞穴（延伸點）',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['緩解腰部下段緊繃','大腸俞穴（延伸點）'],vi:[]},
    location:{zh:'位於背部，第4腰椎棘突下旁開處',vi:''},
    benefit:{zh:'緩解腰部下段緊繃',vi:''},
    technique:{zh:'以掌根按壓',vi:''},
    caution:{zh:'手法柔和，緩解長期久坐導致的腰椎張力',vi:''}},
  {id:'hip_1',bodyPart:'hip',
    name:{zh:'環跳穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['疏通經絡','利腰腿','緩解臀部及大腿後側痠痛','環跳穴','環跳'],vi:[]},
    location:{zh:'位於臀部，股骨大轉子高點與骶管裂孔連線的外三分之一與中三分之一交點處',vi:''},
    benefit:{zh:'疏通經絡、利腰腿、緩解臀部及大腿後側痠痛',vi:''},
    technique:{zh:'以大拇指或肘尖深層按壓，或以掌根揉摩',vi:''},
    caution:{zh:'此穴較深，操作時應確認肌肉鬆弛，避免強力撞擊骨骼',vi:''}},
  {id:'hip_2',bodyPart:'hip',
    name:{zh:'居髎穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['舒筋活絡','強腰膝','緩解臀部外側及大腿外側痠脹','居髎穴','居髎'],vi:[]},
    location:{zh:'位於臀部，髂前上棘與股骨大轉子連線的中點',vi:''},
    benefit:{zh:'舒筋活絡、強腰膝、緩解臀部外側及大腿外側痠脹',vi:''},
    technique:{zh:'以大拇指或掌根按壓揉動',vi:''},
    caution:{zh:'手法應穩定，避開髂骨過硬區',vi:''}},
  {id:'hip_3',bodyPart:'hip',
    name:{zh:'秩邊穴（臀部延伸）',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['舒筋活絡','緩解臀部及大腿後側放射痛','秩邊穴（臀部延伸）'],vi:[]},
    location:{zh:'位於臀部，第4骶椎棘突下旁開三寸',vi:''},
    benefit:{zh:'舒筋活絡、緩解臀部及大腿後側放射痛',vi:''},
    technique:{zh:'以大拇指深層按揉',vi:''},
    caution:{zh:'注意避開坐骨神經走行，力道由輕漸重',vi:''}},
  {id:'hip_4',bodyPart:'hip',
    name:{zh:'承扶穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['舒筋活絡','利腰腿','緩解臀下部與大腿後側緊繃','承扶穴','承扶'],vi:[]},
    location:{zh:'位於臀部橫紋中央',vi:''},
    benefit:{zh:'舒筋活絡、利腰腿、緩解臀下部與大腿後側緊繃',vi:''},
    technique:{zh:'以大拇指或掌根深度按揉，配合下肢伸展',vi:''},
    caution:{zh:'此處為大腿後側肌肉啟動點，按壓時應注意是否有神經壓迫感',vi:''}},
  {id:'hip_5',bodyPart:'hip',
    name:{zh:'殷門穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['強筋健骨','通經活絡','緩解大腿後側肌肉痠痛','殷門穴','殷門'],vi:[]},
    location:{zh:'位於大腿後側，承扶穴與委中穴連線上，承扶下六寸',vi:''},
    benefit:{zh:'強筋健骨、通經活絡、緩解大腿後側肌肉痠痛',vi:''},
    technique:{zh:'以掌根或大拇指推按，可配合腿部彎曲動作',vi:''},
    caution:{zh:'避免在大腿後側靜脈曲張明顯處用力',vi:''}},
  {id:'hip_6',bodyPart:'hip',
    name:{zh:'浮郄穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['舒筋活絡','緩解大腿後側膝部上方緊繃','浮郄穴','浮郄'],vi:[]},
    location:{zh:'位於大腿後側，委中穴上1寸，股二頭肌腱內側',vi:''},
    benefit:{zh:'舒筋活絡、緩解大腿後側膝部上方緊繃',vi:''},
    technique:{zh:'以指腹點按',vi:''},
    caution:{zh:'力道適中，避開膕窩神經血管豐富區',vi:''}},
  {id:'hip_7',bodyPart:'hip',
    name:{zh:'委陽穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['舒筋通絡','利三焦','緩解膝後側及大腿外下側緊繃','委陽穴','委陽'],vi:[]},
    location:{zh:'位於膝後區，委中穴外側，股二頭肌腱內側',vi:''},
    benefit:{zh:'舒筋通絡、利三焦、緩解膝後側及大腿外下側緊繃',vi:''},
    technique:{zh:'以指腹輕揉',vi:''},
    caution:{zh:'避開膕窩中心，手法需柔和',vi:''}},
  {id:'hip_8',bodyPart:'hip',
    name:{zh:'風市穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['祛風散寒','通經活絡','緩解大腿外側痠痛與無力','風市穴','風市'],vi:[]},
    location:{zh:'位於大腿外側，直立垂手時，中指尖所指處（外側正中線）',vi:''},
    benefit:{zh:'祛風散寒、通經活絡、緩解大腿外側痠痛與無力',vi:''},
    technique:{zh:'以大拇指或握拳輕敲，亦可深度點按',vi:''},
    caution:{zh:'力道視個人耐受度，避免過度激惹骨膜',vi:''}},
  {id:'hip_9',bodyPart:'hip',
    name:{zh:'中瀆穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['舒筋活絡','利關節','緩解大腿外側肌肉緊繃','中瀆穴','中瀆'],vi:[]},
    location:{zh:'位於大腿外側，風市穴下二寸',vi:''},
    benefit:{zh:'舒筋活絡、利關節、緩解大腿外側肌肉緊繃',vi:''},
    technique:{zh:'以大拇指按揉',vi:''},
    caution:{zh:'手法要穩定，避免滑動傷及皮膚',vi:''}},
  {id:'hip_10',bodyPart:'hip',
    name:{zh:'伏兔穴',vi:''},
    meridian:{zh:'足陽明胃經',vi:''},
    keywords:{zh:['舒筋活絡','通經活絡','緩解大腿前側肌肉僵硬','伏兔穴','伏兔'],vi:[]},
    location:{zh:'位於大腿前側，髂前上棘與髕底外側連線上，髕底上六寸',vi:''},
    benefit:{zh:'舒筋活絡、通經活絡、緩解大腿前側肌肉僵硬',vi:''},
    technique:{zh:'以掌根按壓推揉',vi:''},
    caution:{zh:'避免局部肌肉急性挫傷處用力',vi:''}},
  {id:'hip_11',bodyPart:'hip',
    name:{zh:'陰市穴',vi:''},
    meridian:{zh:'足陽明胃經',vi:''},
    keywords:{zh:['溫經散寒','強膝利關節','緩解大腿前側痠脹','陰市穴','陰市'],vi:[]},
    location:{zh:'位於大腿前側，伏兔穴下三寸',vi:''},
    benefit:{zh:'溫經散寒、強膝利關節、緩解大腿前側痠脹',vi:''},
    technique:{zh:'以指腹點按揉動',vi:''},
    caution:{zh:'力道輕緩，保持均勻',vi:''}},
  {id:'hip_12',bodyPart:'hip',
    name:{zh:'梁丘穴',vi:''},
    meridian:{zh:'足陽明胃經',vi:''},
    keywords:{zh:['疏通胃氣','膝關節止痛','緩解大腿前側至膝蓋的緊繃','梁丘穴','梁丘'],vi:[]},
    location:{zh:'位於大腿前側，髕底上二寸，股外側肌與股直肌肌腱之間',vi:''},
    benefit:{zh:'疏通胃氣、膝關節止痛、緩解大腿前側至膝蓋的緊繃',vi:''},
    technique:{zh:'以大拇指點按',vi:''},
    caution:{zh:'此穴為膝關節保健要穴，按摩時可配合膝部輕微活動',vi:''}},
  {id:'hip_13',bodyPart:'hip',
    name:{zh:'髀關穴',vi:''},
    meridian:{zh:'足陽明胃經',vi:''},
    keywords:{zh:['通經活絡','強腰膝','緩解大腿前側肌肉緊繃','髀關穴','髀關'],vi:[]},
    location:{zh:'位於大腿前側，髂前上棘與髕底外側連線上，縫匠肌肌肉處',vi:''},
    benefit:{zh:'通經活絡、強腰膝、緩解大腿前側肌肉緊繃',vi:''},
    technique:{zh:'以掌根按壓推揉',vi:''},
    caution:{zh:'手法需避開鼠蹊部敏感帶，力道由淺入深',vi:''}},
  {id:'hip_14',bodyPart:'hip',
    name:{zh:'足五里',vi:''},
    meridian:{zh:'足厥陰肝經（延伸區）',vi:''},
    keywords:{zh:['活血化瘀','舒緩大腿內側肌群（非鼠蹊敏感區','僅按大腿內側）','足五里'],vi:[]},
    location:{zh:'位於大腿內側，氣沖穴直下三寸',vi:''},
    benefit:{zh:'活血化瘀、舒緩大腿內側肌群（非鼠蹊敏感區，僅按大腿內側）',vi:''},
    technique:{zh:'以掌根輕柔推摩',vi:''},
    caution:{zh:'注意按摩範圍嚴格控制在內側肌肉，不可誤入鼠蹊皺褶深處',vi:''}},
  {id:'hip_15',bodyPart:'hip',
    name:{zh:'陰廉穴',vi:''},
    meridian:{zh:'足厥陰肝經',vi:''},
    keywords:{zh:['調理肝腎','舒緩大腿內側緊繃','陰廉穴','陰廉'],vi:[]},
    location:{zh:'位於大腿內側，氣沖穴直下二寸',vi:''},
    benefit:{zh:'調理肝腎、舒緩大腿內側緊繃',vi:''},
    technique:{zh:'以指腹輕柔按摩',vi:''},
    caution:{zh:'保持力道和緩',vi:''}},
  {id:'hip_16',bodyPart:'hip',
    name:{zh:'血海穴',vi:''},
    meridian:{zh:'足太陰脾經',vi:''},
    keywords:{zh:['活血化瘀','調經止痛','緩解大腿內側深層肌肉痠脹','血海穴','血海'],vi:[]},
    location:{zh:'位於大腿內側，髕底內側上二寸，股內側肌隆起處',vi:''},
    benefit:{zh:'活血化瘀、調經止痛、緩解大腿內側深層肌肉痠脹',vi:''},
    technique:{zh:'以大拇指點按，力道可適度加強',vi:''},
    caution:{zh:'局部若有靜脈曲張應避開',vi:''}},
  {id:'hip_17',bodyPart:'hip',
    name:{zh:'箕門穴',vi:''},
    meridian:{zh:'足太陰脾經',vi:''},
    keywords:{zh:['疏通經絡','緩解大腿內側肌肉緊張','箕門穴','箕門'],vi:[]},
    location:{zh:'位於大腿內側，血海穴上六寸',vi:''},
    benefit:{zh:'疏通經絡、緩解大腿內側肌肉緊張',vi:''},
    technique:{zh:'以掌根推揉',vi:''},
    caution:{zh:'手法穩定，避開內側靜脈',vi:''}},
  {id:'hip_18',bodyPart:'hip',
    name:{zh:'足三里（上方延伸點）',vi:''},
    meridian:{zh:'足陽明胃經',vi:''},
    keywords:{zh:['緩解腿部整體疲勞','足三里（上方延伸點）'],vi:[]},
    location:{zh:'位於大腿下段前側肌群',vi:''},
    benefit:{zh:'緩解腿部整體疲勞',vi:''},
    technique:{zh:'以掌根由上往下滑推',vi:''},
    caution:{zh:'力度均勻即可',vi:''}},
  {id:'hip_19',bodyPart:'hip',
    name:{zh:'秩邊穴（周邊肌肉點）',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['緩解臀部深層張力','秩邊穴（周邊肌肉點）'],vi:[]},
    location:{zh:'位於臀部肌肉群中',vi:''},
    benefit:{zh:'緩解臀部深層張力',vi:''},
    technique:{zh:'以肘尖或拳面輕柔打圈',vi:''},
    caution:{zh:'需避開尾椎及坐骨神經直接按壓',vi:''}},
  {id:'hip_20',bodyPart:'hip',
    name:{zh:'大腿後側肌群點（非特定經絡）',vi:''},
    meridian:{zh:'特定區域',vi:''},
    keywords:{zh:['舒緩大腿後側整體緊繃','大腿後側肌群點（非特定經絡）'],vi:[]},
    location:{zh:'位於大腿後側正中，殷門與承扶中間區域',vi:''},
    benefit:{zh:'舒緩大腿後側整體緊繃',vi:''},
    technique:{zh:'以掌根大面積揉搓',vi:''},
    caution:{zh:'手法應順著肌肉纖維走向',vi:''}},
  {id:'arm_1',bodyPart:'arm',
    name:{zh:'極泉穴',vi:''},
    meridian:{zh:'手少陰心經',vi:''},
    keywords:{zh:['寬胸寧神','舒緩手臂內側麻木與胸悶','極泉穴','極泉'],vi:[]},
    location:{zh:'位於腋窩頂點，腋動脈搏動處',vi:''},
    benefit:{zh:'寬胸寧神、舒緩手臂內側麻木與胸悶',vi:''},
    technique:{zh:'以指腹輕輕按揉',vi:''},
    caution:{zh:'此處血管神經豐富，嚴禁重壓，僅能輕柔點按',vi:''}},
  {id:'arm_2',bodyPart:'arm',
    name:{zh:'青靈穴',vi:''},
    meridian:{zh:'手少陰心經',vi:''},
    keywords:{zh:['通經活絡','緩解肩臂疼痛','青靈穴','青靈'],vi:[]},
    location:{zh:'位於臂內側，肘橫紋上三寸，肱二頭肌內側溝中',vi:''},
    benefit:{zh:'通經活絡、緩解肩臂疼痛',vi:''},
    technique:{zh:'以大拇指點按',vi:''},
    caution:{zh:'力道適中，避開靜脈',vi:''}},
  {id:'arm_3',bodyPart:'arm',
    name:{zh:'少海穴',vi:''},
    meridian:{zh:'手少陰心經',vi:''},
    keywords:{zh:['寧心安神','疏通經絡','緩解肘臂麻木','少海穴','少海'],vi:[]},
    location:{zh:'位於肘橫紋內側端，肱骨內上髁與肱二頭肌腱之間凹陷處',vi:''},
    benefit:{zh:'寧心安神、疏通經絡、緩解肘臂麻木',vi:''},
    technique:{zh:'以大拇指點按',vi:''},
    caution:{zh:'力道輕緩，避開麻感太強的神經',vi:''}},
  {id:'arm_4',bodyPart:'arm',
    name:{zh:'曲澤穴',vi:''},
    meridian:{zh:'手厥陰心包經',vi:''},
    keywords:{zh:['清熱除煩','寬胸止嘔','緩解前臂緊繃','曲澤穴','曲澤'],vi:[]},
    location:{zh:'位於肘橫紋中，肱二頭肌腱尺側緣',vi:''},
    benefit:{zh:'清熱除煩、寬胸止嘔、緩解前臂緊繃',vi:''},
    technique:{zh:'以大拇指按壓',vi:''},
    caution:{zh:'按壓時避免用力過猛引起放射感',vi:''}},
  {id:'arm_5',bodyPart:'arm',
    name:{zh:'尺澤穴',vi:''},
    meridian:{zh:'手太陰肺經',vi:''},
    keywords:{zh:['宣肺解表','清熱止咳','緩解手臂前側緊繃','尺澤穴','尺澤'],vi:[]},
    location:{zh:'位於肘橫紋中，肱二頭肌腱橈側緣凹陷處',vi:''},
    benefit:{zh:'宣肺解表、清熱止咳、緩解手臂前側緊繃',vi:''},
    technique:{zh:'以大拇指按揉',vi:''},
    caution:{zh:'手法要輕柔',vi:''}},
  {id:'arm_6',bodyPart:'arm',
    name:{zh:'孔最穴',vi:''},
    meridian:{zh:'手太陰肺經',vi:''},
    keywords:{zh:['清熱止血','潤肺利咽','緩解前臂肌肉痠脹','孔最穴','孔最'],vi:[]},
    location:{zh:'位於前臂掌面，尺澤穴與太淵穴連線上，腕橫紋上七寸',vi:''},
    benefit:{zh:'清熱止血、潤肺利咽、緩解前臂肌肉痠脹',vi:''},
    technique:{zh:'以大拇指點按',vi:''},
    caution:{zh:'手法穩定',vi:''}},
  {id:'arm_7',bodyPart:'arm',
    name:{zh:'內關穴',vi:''},
    meridian:{zh:'手厥陰心包經',vi:''},
    keywords:{zh:['寧心安神','和胃止嘔','緩解前臂及胸部不適','內關穴','內關'],vi:[]},
    location:{zh:'位於前臂掌側，腕橫紋上二寸，兩筋之間',vi:''},
    benefit:{zh:'寧心安神、和胃止嘔、緩解前臂及胸部不適',vi:''},
    technique:{zh:'以大拇指深層點按',vi:''},
    caution:{zh:'此穴功效廣泛，為手臂按摩必按大穴',vi:''}},
  {id:'arm_8',bodyPart:'arm',
    name:{zh:'外關穴',vi:''},
    meridian:{zh:'手少陽三焦經',vi:''},
    keywords:{zh:['清熱解表','通經活絡','緩解手臂背側痠痛','外關穴','外關'],vi:[]},
    location:{zh:'位於前臂背側，腕橫紋上二寸，尺骨與橈骨之間',vi:''},
    benefit:{zh:'清熱解表、通經活絡、緩解手臂背側痠痛',vi:''},
    technique:{zh:'以大拇指與食指對捏點按',vi:''},
    caution:{zh:'手法需均勻',vi:''}},
  {id:'arm_9',bodyPart:'arm',
    name:{zh:'手三里',vi:''},
    meridian:{zh:'手陽明大腸經',vi:''},
    keywords:{zh:['調理腸胃','疏經通絡','緩解前臂外側疲勞','手三里'],vi:[]},
    location:{zh:'位於前臂背面，陽溪與曲池連線上，肘橫紋下二寸',vi:''},
    benefit:{zh:'調理腸胃、疏經通絡、緩解前臂外側疲勞',vi:''},
    technique:{zh:'以掌根推揉或指腹點按',vi:''},
    caution:{zh:'力道適中，緩解痠脹感',vi:''}},
  {id:'arm_10',bodyPart:'arm',
    name:{zh:'曲池穴',vi:''},
    meridian:{zh:'手陽明大腸經',vi:''},
    keywords:{zh:['清熱解表','疏通經絡','緩解手臂整體緊繃與過敏症狀','曲池穴','曲池'],vi:[]},
    location:{zh:'位於肘橫紋外側端，屈肘時凹陷處',vi:''},
    benefit:{zh:'清熱解表、疏通經絡、緩解手臂整體緊繃與過敏症狀',vi:''},
    technique:{zh:'以大拇指或掌根按壓',vi:''},
    caution:{zh:'此處肌層較厚，力道可適度加強',vi:''}},
  {id:'arm_11',bodyPart:'arm',
    name:{zh:'臂臑穴',vi:''},
    meridian:{zh:'手陽明大腸經',vi:''},
    keywords:{zh:['舒筋活絡','利肩胛','緩解上臂部痠痛','臂臑穴','臂臑'],vi:[]},
    location:{zh:'位於上臂外側，三角肌下端，曲池穴上七寸',vi:''},
    benefit:{zh:'舒筋活絡、利肩胛、緩解上臂部痠痛',vi:''},
    technique:{zh:'以指腹點按揉動',vi:''},
    caution:{zh:'避開局部骨骼硬點',vi:''}},
  {id:'arm_12',bodyPart:'arm',
    name:{zh:'手五里',vi:''},
    meridian:{zh:'手陽明大腸經',vi:''},
    keywords:{zh:['通經活絡','理氣散結','緩解上臂痠脹','手五里'],vi:[]},
    location:{zh:'位於上臂外側，曲池穴上三寸',vi:''},
    benefit:{zh:'通經活絡、理氣散結、緩解上臂痠脹',vi:''},
    technique:{zh:'以大拇指點按',vi:''},
    caution:{zh:'避開靜脈',vi:''}},
  {id:'arm_13',bodyPart:'arm',
    name:{zh:'消濼穴',vi:''},
    meridian:{zh:'手少陽三焦經',vi:''},
    keywords:{zh:['通經活絡','緩解上臂疼痛','消濼穴','消濼'],vi:[]},
    location:{zh:'位於上臂外側，清冷淵與臑會之間',vi:''},
    benefit:{zh:'通經活絡、緩解上臂疼痛',vi:''},
    technique:{zh:'以指腹按揉',vi:''},
    caution:{zh:'力道輕緩',vi:''}},
  {id:'arm_14',bodyPart:'arm',
    name:{zh:'天井穴',vi:''},
    meridian:{zh:'手少陽三焦經',vi:''},
    keywords:{zh:['清熱散結','舒緩肘部及手臂緊繃','天井穴','天井'],vi:[]},
    location:{zh:'位於肘尖上一寸凹陷處',vi:''},
    benefit:{zh:'清熱散結、舒緩肘部及手臂緊繃',vi:''},
    technique:{zh:'以指腹點按',vi:''},
    caution:{zh:'避開骨骼直接按壓',vi:''}},
  {id:'arm_15',bodyPart:'arm',
    name:{zh:'支溝穴',vi:''},
    meridian:{zh:'手少陽三焦經',vi:''},
    keywords:{zh:['通腑理氣','舒緩前臂背側肌肉疲勞','支溝穴','支溝'],vi:[]},
    location:{zh:'位於前臂背側，腕橫紋上三寸，兩骨之間',vi:''},
    benefit:{zh:'通腑理氣、舒緩前臂背側肌肉疲勞',vi:''},
    technique:{zh:'以大拇指點按',vi:''},
    caution:{zh:'配合腕部輕微轉動',vi:''}},
  {id:'arm_16',bodyPart:'arm',
    name:{zh:'陽池穴',vi:''},
    meridian:{zh:'手少陽三焦經',vi:''},
    keywords:{zh:['舒筋活絡','緩解腕部及手臂背側痠痛','陽池穴','陽池'],vi:[]},
    location:{zh:'位於腕背橫紋中，指總伸肌腱尺側緣凹陷處',vi:''},
    benefit:{zh:'舒筋活絡、緩解腕部及手臂背側痠痛',vi:''},
    technique:{zh:'以指腹輕揉',vi:''},
    caution:{zh:'避開腕骨處過硬區域',vi:''}},
  {id:'arm_17',bodyPart:'arm',
    name:{zh:'大陵穴',vi:''},
    meridian:{zh:'手厥陰心包經',vi:''},
    keywords:{zh:['清心安神','緩解腕部及前臂掌面壓力','大陵穴','大陵'],vi:[]},
    location:{zh:'位於腕橫紋中央，兩筋之間',vi:''},
    benefit:{zh:'清心安神、緩解腕部及前臂掌面壓力',vi:''},
    technique:{zh:'以大拇指點按',vi:''},
    caution:{zh:'力度需適中，避開掌側正中神經',vi:''}},
  {id:'arm_18',bodyPart:'arm',
    name:{zh:'太淵穴',vi:''},
    meridian:{zh:'手太陰肺經',vi:''},
    keywords:{zh:['止咳平喘','補益肺氣','緩解腕部及前臂緊繃','太淵穴','太淵'],vi:[]},
    location:{zh:'位於腕掌側橫紋橈側，橈動脈搏動處',vi:''},
    benefit:{zh:'止咳平喘、補益肺氣、緩解腕部及前臂緊繃',vi:''},
    technique:{zh:'以指腹輕揉',vi:''},
    caution:{zh:'此處靠近動脈，嚴禁粗暴重壓，僅適度輕觸',vi:''}},
  {id:'arm_19',bodyPart:'arm',
    name:{zh:'合谷穴',vi:''},
    meridian:{zh:'手陽明大腸經',vi:''},
    keywords:{zh:['清熱解表','鎮痛','緩解整體手臂與頭部壓力','合谷穴','合谷'],vi:[]},
    location:{zh:'位於手背，第一、二掌骨之間，約第二掌骨中點橈側緣',vi:''},
    benefit:{zh:'清熱解表、鎮痛、緩解整體手臂與頭部壓力',vi:''},
    technique:{zh:'以拇指與食指對捏點按',vi:''},
    caution:{zh:'孕婦禁按（此穴位強刺激可誘發宮縮）',vi:''}},
  {id:'arm_20',bodyPart:'arm',
    name:{zh:'偏歷穴',vi:''},
    meridian:{zh:'手陽明大腸經',vi:''},
    keywords:{zh:['通經活絡','緩解前臂肌肉緊繃','偏歷穴','偏歷'],vi:[]},
    location:{zh:'位於前臂背面，陽溪與曲池連線上，腕橫紋上三寸',vi:''},
    benefit:{zh:'通經活絡、緩解前臂肌肉緊繃',vi:''},
    technique:{zh:'以指腹點按揉動',vi:''},
    caution:{zh:'力道均勻',vi:''}},
  {id:'head_1',bodyPart:'head',
    name:{zh:'百會穴',vi:''},
    meridian:{zh:'督脈',vi:''},
    keywords:{zh:['提神醒腦','升陽舉陷','緩解頭暈與壓力性頭痛','百會穴','百會'],vi:[]},
    location:{zh:'位於頭頂正中線，前後髮際連線的中點（兩耳尖連線與正中線交叉處）',vi:''},
    benefit:{zh:'提神醒腦、升陽舉陷、緩解頭暈與壓力性頭痛',vi:''},
    technique:{zh:'以手指指腹輕柔按壓或輕扣',vi:''},
    caution:{zh:'此處骨骼較薄，力道宜輕，避免過度重壓',vi:''}},
  {id:'head_2',bodyPart:'head',
    name:{zh:'印堂穴',vi:''},
    meridian:{zh:'經外奇穴',vi:''},
    keywords:{zh:['鎮靜安神','清頭明目','緩解眉頭緊繃與緊張性頭痛','印堂穴','印堂'],vi:[]},
    location:{zh:'位於面部，兩眉頭連線中點',vi:''},
    benefit:{zh:'鎮靜安神、清頭明目、緩解眉頭緊繃與緊張性頭痛',vi:''},
    technique:{zh:'以大拇指腹輕推或點按',vi:''},
    caution:{zh:'按摩時應閉眼放鬆，力道輕柔',vi:''}},
  {id:'head_3',bodyPart:'head',
    name:{zh:'太陽穴',vi:''},
    meridian:{zh:'經外奇穴',vi:''},
    keywords:{zh:['清頭明目','止痛醒腦','緩解側頭部緊張','太陽穴','太陽'],vi:[]},
    location:{zh:'位於頭部側面，眉梢與目外眥之間，向後約一寸凹陷處',vi:''},
    benefit:{zh:'清頭明目、止痛醒腦、緩解側頭部緊張',vi:''},
    technique:{zh:'以大拇指或食指指腹輕輕揉按',vi:''},
    caution:{zh:'此處骨骼薄，力度適中，避免造成眼部不適',vi:''}},
  {id:'head_4',bodyPart:'head',
    name:{zh:'風池穴（頭部延伸）',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['平肝熄風','清頭明目','緩解頭痛與頸部痠痛','風池穴（頭部延伸）'],vi:[]},
    location:{zh:'位於後頸部，枕骨之下，胸鎖乳突肌與斜方肌上端之間的凹陷處',vi:''},
    benefit:{zh:'平肝熄風、清頭明目、緩解頭痛與頸部痠痛',vi:''},
    technique:{zh:'以雙手大拇指向上托揉',vi:''},
    caution:{zh:'避免力度過大，以免造成頭暈',vi:''}},
  {id:'head_5',bodyPart:'head',
    name:{zh:'翳風穴',vi:''},
    meridian:{zh:'手少陽三焦經',vi:''},
    keywords:{zh:['聰耳開竅','疏通經絡','緩解耳鳴與頭面部緊張','翳風穴','翳風'],vi:[]},
    location:{zh:'位於頭部，耳垂後方，乳突與下頜角之間的凹陷處',vi:''},
    benefit:{zh:'聰耳開竅、疏通經絡、緩解耳鳴與頭面部緊張',vi:''},
    technique:{zh:'以食指指腹點按',vi:''},
    caution:{zh:'此處有神經分布，力道要輕柔，不可用力衝擊',vi:''}},
  {id:'head_6',bodyPart:'head',
    name:{zh:'率谷穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['清熱止痛','平肝熄風','緩解偏頭痛與頭部壓力','率谷穴','率谷'],vi:[]},
    location:{zh:'位於頭部側面，耳尖直上入髮際一寸五分處',vi:''},
    benefit:{zh:'清熱止痛、平肝熄風、緩解偏頭痛與頭部壓力',vi:''},
    technique:{zh:'以手指指腹揉按',vi:''},
    caution:{zh:'力度均勻，緩慢繞圈',vi:''}},
  {id:'head_7',bodyPart:'head',
    name:{zh:'神庭穴',vi:''},
    meridian:{zh:'督脈',vi:''},
    keywords:{zh:['安神定志','清頭明目','緩解頭暈與焦慮','神庭穴','神庭'],vi:[]},
    location:{zh:'位於頭部正中線，入前髮際五分處',vi:''},
    benefit:{zh:'安神定志、清頭明目、緩解頭暈與焦慮',vi:''},
    technique:{zh:'以指腹輕輕揉按',vi:''},
    caution:{zh:'手法柔和，避開頭皮損傷處',vi:''}},
  {id:'head_8',bodyPart:'head',
    name:{zh:'頭臨泣',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['清頭明目','緩解頭部沉重感','頭臨泣'],vi:[]},
    location:{zh:'位於頭部，前髮際直上五分，神庭旁開三寸（在瞳孔直上）',vi:''},
    benefit:{zh:'清頭明目、緩解頭部沉重感',vi:''},
    technique:{zh:'以指腹輕壓揉動',vi:''},
    caution:{zh:'力度適中',vi:''}},
  {id:'head_9',bodyPart:'head',
    name:{zh:'頭維穴',vi:''},
    meridian:{zh:'足陽明胃經',vi:''},
    keywords:{zh:['清熱散風','緩解額顳部頭痛與眼睛疲勞','頭維穴','頭維'],vi:[]},
    location:{zh:'位於頭部側面，額角髮際直上一寸',vi:''},
    benefit:{zh:'清熱散風、緩解額顳部頭痛與眼睛疲勞',vi:''},
    technique:{zh:'以指腹輕揉按壓',vi:''},
    caution:{zh:'此處結構敏感，力道要溫和',vi:''}},
  {id:'head_10',bodyPart:'head',
    name:{zh:'曲差穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['清頭明目','理氣消腫','曲差穴','曲差'],vi:[]},
    location:{zh:'位於頭部，前髮際直上五分，旁開一寸半',vi:''},
    benefit:{zh:'清頭明目、理氣消腫',vi:''},
    technique:{zh:'以指腹輕柔按摩',vi:''},
    caution:{zh:'力度適中，平緩進行',vi:''}},
  {id:'head_11',bodyPart:'head',
    name:{zh:'通天穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['清頭明目','宣通鼻竅','通天穴','通天'],vi:[]},
    location:{zh:'位於頭頂部，前髮際直上四寸，旁開一寸半',vi:''},
    benefit:{zh:'清頭明目、宣通鼻竅',vi:''},
    technique:{zh:'以指腹輕柔點按',vi:''},
    caution:{zh:'力道要輕，避免不適感',vi:''}},
  {id:'head_12',bodyPart:'head',
    name:{zh:'懸顱穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['清熱止痛','緩解偏頭痛','懸顱穴','懸顱'],vi:[]},
    location:{zh:'位於頭部側面，曲鬢與頭維連線的中點',vi:''},
    benefit:{zh:'清熱止痛、緩解偏頭痛',vi:''},
    technique:{zh:'以指腹輕揉',vi:''},
    caution:{zh:'力度均勻',vi:''}},
  {id:'head_13',bodyPart:'head',
    name:{zh:'角孫穴',vi:''},
    meridian:{zh:'手少陽三焦經',vi:''},
    keywords:{zh:['清熱散風','明目消腫','角孫穴','角孫'],vi:[]},
    location:{zh:'位於頭部側面，摺耳取穴，耳尖正對入髮際處',vi:''},
    benefit:{zh:'清熱散風、明目消腫',vi:''},
    technique:{zh:'以指腹輕按揉動',vi:''},
    caution:{zh:'力度輕柔',vi:''}},
  {id:'head_14',bodyPart:'head',
    name:{zh:'天衝穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['鎮驚安神','緩解頭部緊繃','天衝穴','天衝'],vi:[]},
    location:{zh:'位於頭部側面，耳根後上方入髮際二寸',vi:''},
    benefit:{zh:'鎮驚安神、緩解頭部緊繃',vi:''},
    technique:{zh:'以指腹揉按',vi:''},
    caution:{zh:'避開過硬骨骼直接壓迫',vi:''}},
  {id:'head_15',bodyPart:'head',
    name:{zh:'目窗穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['清頭明目','疏散風熱','目窗穴','目窗'],vi:[]},
    location:{zh:'位於頭部，前髮際直上一寸五分，旁開二寸三分（瞳孔直上）',vi:''},
    benefit:{zh:'清頭明目、疏散風熱',vi:''},
    technique:{zh:'以指腹輕揉',vi:''},
    caution:{zh:'力度溫和，緩解眼部疲勞',vi:''}},
  {id:'head_16',bodyPart:'head',
    name:{zh:'正營穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['清熱散風','止痛','正營穴','正營'],vi:[]},
    location:{zh:'位於頭部，目窗穴後一寸五分',vi:''},
    benefit:{zh:'清熱散風、止痛',vi:''},
    technique:{zh:'以指腹輕揉',vi:''},
    caution:{zh:'手法要輕巧',vi:''}},
  {id:'head_17',bodyPart:'head',
    name:{zh:'腦戶穴',vi:''},
    meridian:{zh:'督脈',vi:''},
    keywords:{zh:['安神定驚','清頭明目','腦戶穴','腦戶'],vi:[]},
    location:{zh:'位於後頭部，枕外隆突上緣凹陷處',vi:''},
    benefit:{zh:'安神定驚、清頭明目',vi:''},
    technique:{zh:'以指腹輕柔揉按',vi:''},
    caution:{zh:'力度適中，避免衝擊枕骨',vi:''}},
  {id:'head_18',bodyPart:'head',
    name:{zh:'玉枕穴',vi:''},
    meridian:{zh:'足太陽膀胱經',vi:''},
    keywords:{zh:['清頭明目','緩解頭部與後頸痠痛','玉枕穴','玉枕'],vi:[]},
    location:{zh:'位於後頭部，枕外隆突上緣旁開一寸三分',vi:''},
    benefit:{zh:'清頭明目、緩解頭部與後頸痠痛',vi:''},
    technique:{zh:'以指腹輕托按摩',vi:''},
    caution:{zh:'力度輕柔，順著枕骨邊緣',vi:''}},
  {id:'head_19',bodyPart:'head',
    name:{zh:'本神穴',vi:''},
    meridian:{zh:'足少陽膽經',vi:''},
    keywords:{zh:['安神定驚','舒緩頭腦緊繃','本神穴','本神'],vi:[]},
    location:{zh:'位於頭部，前髮際直上五分，神庭旁開三寸（瞳孔直上）',vi:''},
    benefit:{zh:'安神定驚、舒緩頭腦緊繃',vi:''},
    technique:{zh:'以指腹輕揉',vi:''},
    caution:{zh:'力度適中',vi:''}},
  {id:'head_20',bodyPart:'head',
    name:{zh:'絲竹空',vi:''},
    meridian:{zh:'手少陽三焦經',vi:''},
    keywords:{zh:['疏風清熱','明目止痛','緩解眉頭眼周壓力','絲竹空'],vi:[]},
    location:{zh:'位於面部，眉梢凹陷處',vi:''},
    benefit:{zh:'疏風清熱、明目止痛、緩解眉頭眼周壓力',vi:''},
    technique:{zh:'以指腹輕輕點按或由內向外推按',vi:''},
    caution:{zh:'此處為眼周敏感區，嚴禁重壓或粗暴摩擦',vi:''}},
];
