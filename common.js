...
async function gasTranslate(text){
  const r=await gasCall('translateNotice',{text:text},40000);
  return r||{ok:false,error:'GAS無回應'};
}
// AI分析公告:內容→分類/標籤/標題/摘要/越南文
async function gasAnalyze(content){
  const r=await gasCall('analyzeNotice',{content:content},40000);
  return r||{ok:false,error:'GAS無回應'};
}
