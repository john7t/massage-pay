...
/* ══════════ Supervisor levels ══════════ */
const SUP_LEVELS=[{id:1,zh:'副理',vi:'Phó quản lý'},{id:2,zh:'經理',vi:'Quản lý'},{id:3,zh:'區經理',vi:'Quản lý khu vực'},{id:4,zh:'執行長',vi:'Giám đốc'}];
...
function addBooking(code,bk){const list=getBookings(code);bk.id=uid();bk.createdAt=Date.now();list.push(bk);saveBookings(code,list);if(bk.customerId){const cs=getCustomers(code);const ci=cs.findIndex(x=>x.id===bk.customerId);if(ci>=0){cs[ci].lastServedAt=bk.datetime||Date.now();saveCustomers(code,cs)}}return bk.id}
function updateBooking(code,id,patch){const list=getBookings(code);const i=list.findIndex(b=>b.id===id);if(i<0)return false;list[i]={...list[i],...patch};saveBookings(code,list);return true}
...
global.MP={
  ...
  getCustomers,saveCustomers,getBookings,saveBookings,uid,upsertCustomer,normPhone,searchCustomers,addBooking,updateBooking,deleteBooking,BOOK_TITLES,bookTitleName,
