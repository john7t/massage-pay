// app-core.js v1.0-036 — 主程式核心元件(登入驗證/首頁/月報表/彈窗),從index.html拆分出來
...
function HomePage({settings,t,refreshKey,onGotoProfile,onGotoNotices,onGotoBook,onGotoChart,onGotoSuggest,onGotoViolation,onGotoBackup,onGotoManage,onGotoMonthly,onGotoCustomers,onGotoSettings,settingsAlert,onUpdateSettings,onLogout}){
  ...
  const updSlipHome=(slipId,patch)=>{const md=JSON.parse(JSON.stringify(data));const dd=md.days[td];dd.slips=(dd.slips||[]).map(s=>s.id===slipId?{...s,...patch}:s);dd.total=slipUnitsTotal(dd.slips);dd.laodian=slipLaodianTotal(dd.slips);stamp(dd);md.days[td]=dd;setData({...md});LS.set(dk(settings.code,ty,tm),md);
    // 跟DayEditor存檔時一樣,客戶姓名/電話有填就同步進客戶資料庫,不然客管頁面不會出現這筆客人
    const updated=dd.slips.find(s=>s.id===slipId);
    if(updated&&settings.code&&((updated.custName||'').trim()||(updated.custPhone||'').trim())){try{upsertCust(settings.code,updated)}catch(_e){}}
  };
  ...
  return(<div className="p-5 max-w-md mx-auto fi">
    ...
    {homeEditSlipId&&(()=>{const s=(todayData.slips||[]).find(x=>x.id===homeEditSlipId);if(!s)return null;return(<div className="fixed inset-0 z-50 bg-black/80 flex items-end sm:items-center justify-center" onClick={()=>setHomeEditSlipId(null)}><div className="su bg-gray-900 w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
      <div className="p-4 border-b border-white/[0.06] flex items-center justify-between"><h3 className="text-base font-bold text-gray-100">{t.slipList}</h3><button onClick={()=>setHomeEditSlipId(null)} className="w-8 h-8 rounded-full bg-white/[0.06] flex items-center justify-center text-gray-400 active:bg-white/[0.12]">✕</button></div>
