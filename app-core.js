// app-core.js v1.0-043 — 主程式核心元件(登入驗證/首頁/月報表/彈窗),從index.html拆分出來
...
function Onboarding({onComplete}){
  ...
  const doLogin=async()=>{
    ...
    try{
      ...
      setChecking(false);
      if(r&&r.ok){doActivate({code:r.code||code,unitPrice:r.unitPrice||250,unitPriceHistory:r.unitPriceHistory||[],lang:r.lang||lang,store:r.store||store,year:bizParts().y,deviceId:devId,gender:r.gender||'',workStart:r.workStart||'',workEnd:r.workEnd||'',shift:r.shift||'',lockPwd:loginPwdInput})}
...
function InfoEditModal({type,settings,t,onClose,onUpdateSettings,onLogout}){
  ...
  return(<div className="fixed inset-0 z-50 bg-black/80 flex items-end sm:items-center justify-center" onClick={onClose}><div className="bg-gray-900 w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl max-h-[85vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
    ...
    <div className="p-4 space-y-3">
      ...
      {priceAskModal&&(<div className="fixed inset-0 z-[60] bg-black/80 flex items-end sm:items-center justify-center" onClick={()=>setPriceAskModal(null)}><div className="bg-gray-900 w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl" onClick={e=>e.stopPropagation()}>
