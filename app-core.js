// app-core.js v1.0-014 — 主程式核心元件(登入驗證/首頁/月報表/彈窗),從index.html拆分出來
...
function PinKeypadCompact({onDigit,onBackspace}){
  const BtnCls="w-9 h-11 rounded-lg bg-white/[0.08] text-gray-100 text-base font-semibold active:opacity-80 flex items-center justify-center flex-shrink-0";
...
function SlipEditFields({s,t,editorCode,custQuery,setCustQuery,onUpdate,onDelete}){
  ...
  return(<div className="px-3 pb-3 pt-1 space-y-2 fi border-t border-white/[0.04]">
    ...
    <div className="grid grid-cols-2 gap-2"><div><label className="text-[10px] text-gray-600 mb-0.5 block">{t.svcLabel}</label><select value={s.svc||''} onChange={e=>onUpdate({svc:e.target.value})} className="w-full bg-white/[0.06] border border-white/[0.08] rounded-lg px-2 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-amber-500"><option value="">--</option>{SERVICES.map(o=><option key={o.code} value={o.code}>{o.code}（{o.min}{t===T.zh?'分':'p'}）</option>)}</select></div><div><label className="text-[10px] text-gray-600 mb-0.5 block">{t.addHour}</label><select value={s.extra||0} onChange={e=>onUpdate({extra:parseInt(e.target.value)||0})} className="w-full bg-white/[0.06] border border-white/[0.08] rounded-lg px-2 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-amber-500">{[0,1,2,3,4,5,6,7,8].map(n=><option key={n} value={n}>+{n}</option>)}</select></div></div>
