...
function Onboarding({onComplete}){
  ...
  const ticketBlock=(prefixKey,titleKey,onApproved)=>(
    <div className="space-y-4">
      ...
      <button onClick={()=>{const base=location.origin+location.pathname.replace(/index\.html$/,'')+'auth.html';const link=buildReqLink(base,ticketSeq);const zhMsg=(T.zh[prefixKey]||'').replace('{code}',code);const viMsg=(T.vi[prefixKey]||'').replace('{code}',code);const isVi=ticketSeq&&ticketSeq[1]==='2';const msg=zhMsg+(isVi?'\n'+viMsg:'')+'\n'+link;location.href='https://line.me/R/share?text='+encodeURIComponent(msg)}} className="w-full px-3 py-4 rounded-xl text-base font-bold bg-[#06C755] text-white active:bg-[#05b34c]">{t.ticketSend}</button>
