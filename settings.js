...
function BackupSection({settings,t}){
  ...
  const doShareCode=()=>{
    if(!expCode)return;
    try{
      liff.shareTargetPicker([{type:'text',text:'我的資料範圍代碼（'+settings.year+'年 '+(expRange?expRange.first+'~'+expRange.last+'月':'')+'）：\n'+expCode}]);
    }catch(e){alert('分享失敗，請改用複製按鈕：'+e)}
  };
