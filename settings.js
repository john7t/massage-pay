        ...
        {(()=>{const ghLocal=getGHConfig();const hasConn=!!(ghLocal&&ghLocal.token);return(
          <div className="space-y-2"><label className="text-xs text-gray-400 mb-1 block">{t.serviceStatus}</label>
            ...
            {connMode==='apply'&&(<div className="space-y-3 fi">
              ...
              <p className="text-[10px] text-gray-600 text-center font-mono select-all break-all">{settings.code}:{devId}</p><button onClick={()=>{const link=buildReqLink(location.origin+location.pathname.replace(/index\.html$/,'')+'auth.html',genConnReq(settings.code,devId));sendTicketFlex('🔗 連線申請',t.lineConnReqPrefix,link)}} className="w-full py-2.5 rounded-xl text-white text-sm font-bold bg-[#06C755] active:bg-[#05b34c]">{t.sendViaLine}</button>
