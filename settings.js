...
// v1.12-021 / 公告列表版面調整:第一排只留摘要+編輯鈕(主分類移出);第二排右側改放主/子分類(合併顯示,如"薪資・調整")+開啟次數(👁圖示+數字,取代原本的已讀人數,開啟次數是更寬鬆的被動記錄指標) | 前: 語系選項從基資分頁移到首頁設定的主題下方(語系是個人偏好,不屬於需要主管審核的基資,放首頁設定更直覺) | 前: 版次對齊(內容未變動):跟隨common.js/index.html/auth.html/notice-modal.js這次的整理點同步版號,方便日後從標題/檔頭確認全套部署是否一致 | 前: 公告分頁:移除進入公告頁後的提示文字段落(點右上新增公告...那句)。新增公告的權限限制(限主管supervisor+admin)在v034已完成(canManage判斷),此次確認沿用 | 前:hotfix:公告編輯權限判斷修正(admin查admin.cfg雜湊,不是staff.json role) | 前: 新增公告完整流程接功能(033)
...
function CustomerPage({t,settings}){
  ...
  return(<div className="fi space-y-3 max-w-lg mx-auto px-4 py-2">
    ...
    {mode==='search'&&(<div className="space-y-2">
      ...
      <div className="space-y-1.5">{custList.map((c,i)=>{const last=custLastSlip(settings.code,year,c.custName,c.custTitle,c.custPhone);return(<div key={i} onClick={()=>setEditCust({custName:c.custName||'',custTitle:c.custTitle||'',custPhone:c.custPhone||'',pressBody:c.pressBody||'',pressFoot:c.pressFoot||'',parts:c.parts||[]})} className="flex items-center gap-2 px-3 py-2.5 bg-white/[0.03] border border-white/[0.05] rounded-lg active:bg-white/[0.06]"><span className="text-sm text-gray-200">{c.custName||''}</span>{c.custTitle&&<span className="text-xs text-gray-400">{bookTitleName(c.custTitle,t===T.zh?'zh':'vi')}</span>}{c.custPhone&&<span className="text-xs text-gray-500">{c.custPhone}</span>}{last&&last.svc&&<span className="text-xs font-bold text-amber-300">{slipSvcLabel(last,t===T.zh?"zh":"vi")}</span>}{last&&<span className="text-[11px] text-gray-500 tabular-nums ml-auto whitespace-nowrap">{last.month}/{last.day} {String(new Date(last.startTime).getHours()).padStart(2,'0')}:{String(new Date(last.startTime).getMinutes()).padStart(2,'0')}</span>}</div>)})}</div>
    </div>)}
...
function NoticeManagePage({t,settings}){
  ...
  return(<div className="fi">
    ...
    <div className="space-y-2 mt-3">{list.length===0?<p className="text-xs text-gray-600 text-center py-6">{t.noticeEmpty||'目前沒有公告'}</p>:list.slice().reverse().map(n=>(<div key={n.id} onClick={()=>openNotice(n)} className="bg-white/[0.03] border border-white/[0.05] rounded-xl px-3 py-2.5 active:bg-white/[0.06] cursor-pointer"><div className="flex items-center justify-between gap-2"><p className="text-sm text-gray-200 font-medium truncate flex-1">{(noticeSummary?noticeSummary(n,settings.lang):(n.summary||n.title))||n.title}</p>{canManage&&<button onClick={(ev)=>openEdit(n,ev)} className="text-[11px] px-1.5 py-0.5 rounded-full bg-white/[0.08] text-gray-300 active:bg-white/[0.15] flex-shrink-0">✏️</button>}</div><div className="flex items-center justify-between gap-2 mt-1"><span className="text-[10px] text-gray-500 truncate">👤 {n.author||''} · {n.date}</span><div className="flex items-center gap-1.5 flex-shrink-0">{n.cat&&<span className="text-[10px] px-1.5 py-0.5 rounded-full bg-amber-500/15 text-amber-400">{n.cat}{n.subcat?'・'+n.subcat:''}</span>}<span className="text-[10px] text-gray-600">👁 {typeof n.openCount==='number'?n.openCount:0}</span></div></div></div>))}</div>
