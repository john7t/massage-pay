...
function BackupSection({settings,t}){
  ...
  const[lastJson,setLastJson]=useState(''); // 記住最後一次產生的完整備份JSON字串,LINE內建瀏覽器下載常常沒反應,留一份可以直接複製當備援
  const inLiffClient=(()=>{try{return typeof liff!=='undefined'&&liff.isInClient&&liff.isInClient()}catch(_e){return false}})();
  const goBackupPage=()=>{
    const url='https://liff.line.me/2010673151-kemVP4Pi?backupView=1&code='+encodeURIComponent(settings.code)+'&year='+settings.year;
    location.href=url;
  };
  ...
  const doCloudBackup=async()=>{
    ...
    try{
      ...
      if(r&&r.ok){setCloudMsg({ok:true,text:'✓ 已備份到雲端（'+settings.year+'年）'});setCooldownUntil(Date.now()+60000)}
  ...
  const doShareCode=()=>{
    ...
    sendTicketFlex('📋 資料範圍代碼',(expRange?settings.year+'年 '+expRange.first+'~'+expRange.last+'月':'')+'\n'+expCode,indexUrl);
  };
  ...
  // JSON 完整備份匯出
  const doJsonExport=()=>{try{const obj=makePersonalBackup(settings.code,settings.year);const jsonStr=JSON.stringify(obj);setLastJson(jsonStr);const blob=new Blob([jsonStr],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`massage-${settings.code}-${settings.year}-backup.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);doExport();setJsonMsg({ok:true,text:inLiffClient?'已產生備份內容。如果沒有跳出檔案下載提示（LINE內建瀏覽器常常不支援），請改用下面「複製完整備份內容」按鈕':t.jsonAndCodeDone})}catch(e){setJsonMsg({ok:false,text:t.jsonExportFail})}setTimeout(()=>setJsonMsg(null),inLiffClient?9000:5000)};
  const doCopyJson=async()=>{if(!lastJson)return;try{await navigator.clipboard.writeText(lastJson)}catch(_e){};setJsonMsg({ok:true,text:'已複製完整備份內容，可貼到LINE Keep、記事本或傳給自己保存'});setTimeout(()=>setJsonMsg(null),4000)};
  // JSON 匯入
  ...
  return(<div className="space-y-3 pt-4 border-t border-white/[0.06]"><h3 className="text-sm font-semibold text-amber-400">{t.backup}</h3>
    <div className="bg-emerald-500/[0.06] border border-emerald-500/20 rounded-xl p-3 space-y-2">
      <p className="text-sm font-semibold text-emerald-400">☁️ 雲端備份（推薦）</p>
      <p className="text-[11px] text-gray-500 leading-relaxed">換手機前先備份，登入新手機後查詢就能直接還原，不用複製代碼或傳檔案</p>
      <div className="grid grid-cols-2 gap-2">
        <button onClick={doCloudBackup} disabled={cloudBusy} className="py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold disabled:opacity-50">{cloudBusy?'處理中…':'備份到雲端'}</button>
        <button onClick={doCloudCheck} disabled={cloudBusy||cooldownLeft>0} className="py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.1] text-gray-200 text-sm font-semibold disabled:opacity-50">{cooldownLeft>0?`請稍候 ${cooldownLeft}s`:'查詢雲端備份'}</button>
      </div>
      {cloudCheck&&(cloudCheck.found?(
        <div className="bg-white/[0.04] rounded-lg p-2.5 space-y-1.5">
          <p className="text-xs text-gray-300">找到備份：{cloudCheck.year}年，更新於 {cloudCheck.updatedAt}</p>
          {!cloudConfirm?(
            <button onClick={()=>setCloudConfirm(true)} className="w-full py-2 rounded-lg bg-amber-600 text-white text-xs font-bold">還原這份備份</button>
          ):(
            <div className="space-y-1.5"><p className="text-[11px] text-red-400">還原會覆蓋本機同年份的資料，確定嗎？</p><div className="grid grid-cols-2 gap-2"><button onClick={()=>setCloudConfirm(false)} className="py-2 rounded-lg bg-white/[0.06] text-gray-400 text-xs">取消</button><button onClick={doCloudRestore} disabled={cloudBusy} className="py-2 rounded-lg bg-red-600 text-white text-xs font-bold disabled:opacity-50">{cloudBusy?'還原中…':'確定還原'}</button></div></div>
          )}
        </div>
      ):(<p className="text-xs text-gray-500">目前雲端還沒有這個編號的備份</p>))}
      {cloudMsg&&<p className={`text-xs ${cloudMsg.ok?'text-emerald-400':'text-red-400'}`}>{cloudMsg.text}</p>}
    </div>
    <button onClick={doExport} className="w-full py-2.5 bg-amber-600 rounded-xl text-white text-sm font-semibold">{t.exportRangeCode}</button>{expCode&&(<div className="space-y-1"><p className="text-[11px] text-gray-500">{t.rangeHint}：{settings.year} {expRange?expRange.first+'~'+expRange.last+'月':''}</p><div className="relative"><div className="bg-white/[0.04] rounded-xl p-3 text-[10px] text-gray-500 break-all font-mono select-all pr-14">{expCode}</div><div className="absolute top-2 right-2 flex gap-1"><button onClick={doCopy} className="px-2 py-1 bg-white/10 rounded text-xs text-gray-300">📋</button>{inLiffClient&&<button onClick={doShareCode} className="px-2 py-1 bg-[#06C755] rounded text-xs text-white">傳送</button>}</div></div></div>)}<textarea value={input} onChange={e=>setInput(e.target.value)} rows={2} placeholder={t.enterUrl} className="w-full bg-white/[0.06] border border-white/[0.08] rounded-xl px-3 py-2.5 text-sm text-gray-100 font-mono focus:outline-none focus:border-amber-500 resize-none"/><button onClick={doImport} disabled={!input.trim()} className={`w-full py-3 rounded-xl font-bold text-white ${input.trim()?'bg-amber-600':'bg-white/[0.06] text-gray-600 cursor-not-allowed'}`}>{t.importUrl}</button><p className="text-[11px] text-gray-500 leading-relaxed">{t.rangeCodeNote}</p>{result&&<div className={`p-2.5 rounded-xl text-sm text-center fi ${result.includes(t.importFail)||result===t.noData?'bg-red-500/15 text-red-300':'bg-emerald-500/15 text-emerald-300'}`}>{result}</div>}
    <div className="pt-4 mt-2 border-t border-white/[0.06] space-y-2"><h3 className="text-sm font-semibold text-amber-400">{t.fullBackup}</h3><p className="text-[11px] text-gray-500 leading-relaxed">{t.fullBackupDesc}</p>{inLiffClient&&<p className="text-[11px] text-amber-400 leading-relaxed">⚠️ 目前在LINE內建瀏覽器開啟，檔案下載可能不會有提示，建議改用下面「在LINE查看／分享備份」</p>}{inLiffClient&&<button onClick={goBackupPage} className="w-full py-3 rounded-xl bg-[#06C755] text-white text-sm font-bold active:bg-[#05b34c]">📱 在LINE查看／分享備份</button>}<button onClick={doJsonExport} className="w-full py-3 rounded-xl bg-amber-600 text-white text-sm font-bold active:bg-amber-700">{t.jsonExport}</button>{lastJson&&<button onClick={doCopyJson} className="w-full py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.1] text-gray-200 text-sm font-semibold active:bg-white/[0.1]">📋 複製完整備份內容</button>}<button onClick={()=>fileRef.current&&fileRef.current.click()} className="w-full py-3 rounded-xl bg-white/[0.06] border border-white/[0.1] text-gray-200 text-sm font-bold active:bg-white/[0.1]">{t.jsonImport}</button><input ref={fileRef} type="file" accept="application/json,.json" onChange={doJsonImport} className="hidden"/>{pendingImport&&<div className="bg-amber-600/10 border border-amber-500/30 rounded-xl p-3 space-y-2 fi"><p className="text-[13px] text-amber-200">{t.jsonConfirmOverwrite}</p><div className="grid grid-cols-2 gap-2"><button onClick={()=>setPendingImport(null)} className="py-2 rounded-lg bg-white/[0.06] text-gray-300 text-sm">{t.cancel||'取消'}</button><button onClick={confirmImport} className="py-2 rounded-lg bg-amber-600 text-white text-sm font-bold">{t.confirm}</button></div></div>}{jsonMsg&&<div className={`p-2.5 rounded-xl text-sm text-center fi ${jsonMsg.ok?'bg-emerald-500/15 text-emerald-300':'bg-red-500/15 text-red-300'}`}>{jsonMsg.text}</div>}</div>
  </div>)}
...
function CustomerPage({t,settings,onClose}){
  ...
  return(<div className="fi space-y-3 max-w-lg mx-auto px-4 py-2">
    ...
    {mode==='blacklist'&&(<div className="space-y-2">
      ...
      <p className="text-[11px] text-gray-600 text-center pt-2 leading-relaxed">符合條件才會出現在這個分頁：要在流水詳情輸入姓名、稱謂、手機，並勾選拒接及填寫原因</p>
    </div>)}
...
function SettingsPage({settings,onUpdate,t,theme,setTheme,onClose,onLogout}){
  ...
  const sheetHeight=(subTab==='home'||subTab==='notice')?90:50;
  return(
    ...
    <div className="flex h-full fi">
      ...
      {subTab==='home'&&(<div className="space-y-5 fi">
        <div className="sticky top-0 z-10 -mx-4 px-4 py-1 bg-gray-950 flex items-center justify-center"><h2 className="text-sm font-semibold text-gray-300">{t.homeSettings}</h2></div>
