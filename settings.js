...
function BackupSection({settings,t,onUpdate}){
  ...
  const doCloudBackup=async()=>{
    ...
    try{
      ...
      const r=await gasBackupSubmit(settings.code,settings.year,jsonStr);
  ...
  const doCloudCheck=async()=>{
    ...
    try{
      const r=await gasBackupCheck(settings.code);
  ...
  const doCloudRestore=async()=>{
    setCloudBusy(true);setCloudMsg(null);
    try{
      const r=await gasBackupFetch(settings.code);
      if(r&&r.ok){
  ...
  return(<div className="space-y-3 pt-4 border-t border-white/[0.06]"><h3 className="text-sm font-semibold text-amber-400">{t.backup}</h3>
    ...
    <div className="bg-emerald-500/[0.06] border border-emerald-500/20 rounded-xl p-3 space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        <button onClick={doCloudBackup} disabled={cloudBusy||cooldownLeft>0} className="py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold disabled:opacity-50">{cloudBusy?t.cloudBackupProcessing:cooldownLeft>0?t.cloudCheckWait.replace('{0}',cooldownLeft):t.cloudBackupBtn}</button>
        <button onClick={doCloudCheck} disabled={cloudBusy||cooldownLeft>0} className="py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.1] text-gray-200 text-sm font-semibold disabled:opacity-50">{cooldownLeft>0?t.cloudCheckWait.replace('{0}',cooldownLeft):t.cloudCheckBtn}</button>
      </div>
      ...
      <label className="flex items-center gap-2 pt-1 cursor-pointer"><input type="checkbox" checked={!!settings.autoCloudBackup} onChange={e=>onUpdate&&onUpdate({...settings,autoCloudBackup:e.target.checked})} className="accent-emerald-500"/><span className="text-xs text-gray-400">{t.autoBackupLabel}</span></label>
    </div>
  </div>);
}
