...
function BackupSection({settings,t,onUpdate}){
  ...
  // 1.13版:LINE登入,取得userId存本機,雲端備份防撞號用。
  ...
  useEffect(()=>{
    // 本機已經有存userId就不用再跑檢查;沒有的話,元件掛載時嘗試跑一次liff.init(),
    // 抓「剛從LINE登入頁導轉回來」這個情境(liff.login()導轉離開又導回同一個網址,整頁重新載入,靠這個useEffect接住結果)
  ...
  const doLineLogin=async()=>{
    ...
    try{
      await liff.init({liffId:INDEX_LIFF_ID});
      if(liff.isLoggedIn())liff.logout(); // 先登出清掉舊token,確保這次是全新登入,避免快取到缺權限的舊token
  ...
  return(<div className="space-y-3 pt-4 border-t border-white/[0.06]"><h3 className="text-sm font-semibold text-amber-400">{t.backup}</h3>
    ...
    <div className={`bg-emerald-500/[0.06] border border-emerald-500/20 rounded-xl p-3 space-y-2 ${!lineUserId?'opacity-50':''}`}>
      ...
      {!lineUserId&&<p className="text-xs text-amber-500/80">↑ {t.lineLoginRequiredMsg||'請先用LINE登入才能使用雲端備份'}</p>}
      <div className="grid grid-cols-2 gap-2">
        <button onClick={doCloudBackup} disabled={!lineUserId||cloudBusy||cooldownLeft>0} className="py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold disabled:opacity-50">{cloudBusy?t.cloudBackupProcessing:cooldownLeft>0?t.cloudCheckWait.replace('{0}',cooldownLeft):t.cloudBackupBtn}</button>
        <button onClick={doCloudCheck} disabled={!lineUserId||cloudBusy||cooldownLeft>0} className="py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.1] text-gray-200 text-sm font-semibold disabled:opacity-50">{cooldownLeft>0?t.cloudCheckWait.replace('{0}',cooldownLeft):t.cloudCheckBtn}</button>
      </div>
      ...
      <label className="flex items-center gap-2 pt-1 cursor-pointer"><input type="checkbox" checked={!!settings.autoCloudBackup} onChange={e=>onUpdate&&onUpdate({...settings,autoCloudBackup:e.target.checked})} className="accent-emerald-500" disabled={!lineUserId}/><span className="text-xs text-gray-400">{t.autoBackupLabel}</span></label>
    ...
    <div className="flex h-full fi">
      ...
      {subTab==='home'&&(<div className="space-y-5 fi -mt-4">
        ...
        <div className="bg-white/[0.04] rounded-xl px-4 py-3 space-y-2">
            ...
            <label className="flex items-center justify-between gap-2 py-1 cursor-pointer"><span className="text-sm text-gray-300">{t.homeLayoutC}</span><button onClick={()=>upd({homeShowC:form.homeShowC===false})} className={`w-10 h-6 rounded-full flex items-center px-0.5 transition-colors flex-shrink-0 ${form.homeShowC!==false?'bg-amber-600 justify-end':'bg-white/[0.1] justify-start'}`}><span className="w-5 h-5 rounded-full bg-white block"></span></button></label>
      ...
      {subTab==='backup'&&(<div className="space-y-4 fi">
        ...
        <BackupSection settings={settings} t={t} onUpdate={onUpdate}/>
      </div>)}
