  ...
  return(<div className="fi">
    ...
    <div className="space-y-2 mt-3">{list.length===0?<p className="text-xs text-gray-600 text-center py-6">{t.noticeEmpty||'目前沒有公告'}</p>:list.filter(n=>(catTab==='all'||n.cat===catTab)&&(subTabSel==='all'||n.subcat===subTabSel)).slice().reverse().map(n=>{
      ...
      return(<div key={n.id} onClick={()=>openNotice(n)} className={`bg-white/[0.03] border border-white/[0.05] rounded-xl px-3 py-2.5 active:bg-white/[0.06] cursor-pointer ${isOff?'opacity-50':''}`}>
        <div className="flex items-center justify-between gap-2"><p className={`text-sm font-medium truncate flex-1 ${isOff?'text-gray-500':'text-gray-200'}`}>{displayText}</p><div className="flex items-center gap-1 flex-shrink-0">{canManage&&<button onClick={(ev)=>doPushListItem(n.id,ev)} disabled={listPushBusy===n.id} className="text-[11px] px-1.5 py-0.5 rounded-full bg-white/[0.08] text-gray-300 active:bg-white/[0.15] disabled:opacity-50">{listPushBusy===n.id?'…':'📲'}</button>}{canManage&&<button onClick={(ev)=>openEdit(n,ev)} className="text-[11px] px-1.5 py-0.5 rounded-full bg-white/[0.08] text-gray-300 active:bg-white/[0.15] flex-shrink-0">✏️</button>}</div></div>
