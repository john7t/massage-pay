...
function AcupointPage({t}){
  ...
  const results=useMemo(()=>{
    ...
    if(!kw)return ACUPOINTS;
    return ACUPOINTS.filter(a=>{
      if(a.name[lang].toLowerCase().includes(kw))return true;
      if(a.name.zh.toLowerCase().includes(kw))return true; // 不管語系,名稱也順便比對中文,避免打錯語系找不到
    ...
    <div className="space-y-2">
      {results.map(a=>{
        ...
        return(<div key={a.id} className="bg-white/[0.03] border border-white/[0.06] rounded-xl overflow-hidden">
          <div onClick={()=>setOpenId(open?null:a.id)} className="px-3 py-2.5 flex items-center justify-between cursor-pointer active:bg-white/[0.04]"><span className="text-sm font-semibold text-gray-100">{a.name[lang]}</span><span className="text-gray-500 text-xs">{open?'▲':'▼'}</span></div>
          {open&&(<div className="px-3 pb-3 space-y-2 border-t border-white/[0.06] pt-2">
            <div><p className="text-[11px] text-amber-400 font-semibold mb-0.5">{t.acupointLocation}</p><p className="text-[13px] text-gray-300 leading-relaxed">{a.location[lang]}</p></div>
            <div><p className="text-[11px] text-amber-400 font-semibold mb-0.5">{t.acupointBenefit}</p><p className="text-[13px] text-gray-300 leading-relaxed">{a.benefit[lang]}</p></div>
            <div><p className="text-[11px] text-amber-400 font-semibold mb-0.5">{t.acupointTechnique}</p><p className="text-[13px] text-gray-300 leading-relaxed">{a.technique[lang]}</p></div>
            <div><p className="text-[11px] text-red-400 font-semibold mb-0.5">{t.acupointCaution}</p><p className="text-[13px] text-gray-400 leading-relaxed">{a.caution[lang]}</p></div>
          </div>)}
        </div>);
      })}
    </div>
