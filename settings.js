...
function AcupointPage({t}){
  ...
  const results=useMemo(()=>{
    ...
    if(kw){
      arr=arr.filter(a=>{
        ...
        return (a.keywords[lang]||[]).some(k=>k.toLowerCase().includes(kw))||(a.keywords.zh||[]).some(k=>k.toLowerCase().includes(kw));
      });
    }
    return arr;
  ...
  return(<div className="space-y-3 fi">
    ...
    <input value={q} onChange={e=>setQ(e.target.value)} placeholder={t.acupointSearchPh} className="w-full bg-white/[0.06] border border-white/[0.08] rounded-xl px-3 py-2.5 text-sm text-gray-100 focus:outline-none focus:border-amber-500"/>
    ...
    <div className="space-y-2">
      {results.map(a=>{
        ...
        return(<div key={a.id} className="bg-white/[0.03] border border-white/[0.06] rounded-xl overflow-hidden">
          <div onClick={()=>setOpenId(open?null:a.id)} className="px-3 py-2.5 flex items-center justify-between cursor-pointer active:bg-white/[0.04]"><div><span className="text-sm font-semibold text-gray-100">{fb(a.name)}</span><span className="text-[10px] text-sky-400 ml-2">{fb(a.meridian)}</span></div><span className="text-gray-500 text-xs">{open?'▲':'▼'}</span></div>
