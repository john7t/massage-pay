...
function CustomerPage({t,settings,onClose}){
  ...
  const blCandidates=useMemo(()=>{
    ...
    allSlips.filter(s=>s.blacklist&&(s.custName||'').trim()&&(s.custPhone||'').trim()).forEach(s=>{
      const key=(s.custPhone||'')+'|'+(s.custName||'')+'|'+(s.custTitle||'');
      if(seen[key])return;seen[key]=true;
      out.push({key,custName:s.custName,custTitle:s.custTitle,custPhone:s.custPhone,reason:s.blacklistReason||'騷擾'});
    });
    return out;
  },[allSlips]);
  const doBlSubmit=async(cand)=>{
    ...
    try{
      const r=await gasBlacklistSubmit(settings.code,cand.custName,cand.custTitle,cand.custPhone,cand.reason);
