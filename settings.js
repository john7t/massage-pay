...
function CustomerPage({t,settings}){
  ...
  return(<div className="fi space-y-3">
    ...
    {mode==='search'&&(<div className="space-y-2">
      ...
      <div className="space-y-1.5">{custList.map((c,i)=>{const last=custLastSlip(settings.code,year,c.custName,c.custTitle,c.custPhone);return(<div key={i} onClick={()=>setEditCust({custName:c.custName||'',custTitle:c.custTitle||'',custPhone:c.custPhone||'',pressBody:c.pressBody||'',pressFoot:c.pressFoot||'',parts:c.parts||[]})} className="flex items-center gap-2 px-3 py-2.5 bg-white/[0.03] border border-white/[0.05] rounded-lg active:bg-white/[0.06]"><span className="text-sm text-gray-200">{c.custName||''}</span>{c.custTitle&&<span className="text-xs text-gray-400">{bookTitleName(c.custTitle,t===T.zh?'zh':'vi')}</span>}{c.custPhone&&<span className="text-xs text-gray-500">{c.custPhone}</span>}{last&&last.svc&&<span className="text-xs font-bold text-amber-300">{slipSvcLabel(last)}</span>}{last&&<span className="text-[11px] text-gray-500 tabular-nums ml-auto whitespace-nowrap">{last.month}/{last.day} {String(new Date(last.startTime).getHours()).padStart(2,'0')}:{String(new Date(last.startTime).getMinutes()).padStart(2,'0')}</span>}</div>)})}</div>
    </div>)}
