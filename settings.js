...
function AcupointPage({t}){
  ...
  const fb=(obj)=>obj[lang]||obj.zh; // 越南文還沒補的欄位,自動fallback顯示中文
  const[bp,setBp]=useState(ACUPOINT_BODYPARTS[0].id);
  const[meridian,setMeridian]=useState('all');
  const[q,setQ]=useState('');
  const[openId,setOpenId]=useState(null);
  useEffect(()=>{setMeridian('all');setOpenId(null)},[bp]); // 換部位時重置經脈篩選跟展開狀態
  const bpList=useMemo(()=>ACUPOINTS.filter(a=>a.bodyPart===bp),[bp]);
