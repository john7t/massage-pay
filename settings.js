...
function AcupointPage({t}){
  ...
  useEffect(()=>{fetchAcupoints().then(d=>{setData(d);if(d.bodyParts&&d.bodyParts.length)setBp(d.bodyParts[0].id)})},[]);
...
function SettingsPage({settings,onUpdate,t,theme,setTheme,onClose,onLogout}){
  ...
  return(
    ...
    <div className="flex h-full fi">
      ...
      {subTab==='acupoint'&&<AcupointPage t={t}/>}
  ...
  return(
    ...
    <div className="flex h-full fi">
      ...
      {subTab==='home'&&(<div className="space-y-5 fi -mt-4">
        ...
        <div><label className="text-xs text-gray-400 mb-1 block">{t.theme}</label>
          <div className="grid grid-cols-3 gap-2">{[['pink',t.themePink,'#fce4f6',null],['black',t.themeBlack,'#030712',null],['gray',t.themeGray,'#1f2937',null],['white',t.themeWhite,'#f9fafb',null],['smooth',t.themeSmooth,'#0d0d18','linear-gradient(135deg,#0d0d18 0%,#2a1a4a 60%,#7c3aed 100%)']].map(([k,l,c,grad])=>(<button key={k} onClick={()=>setTheme(k)} className={`py-2.5 rounded-xl text-xs font-semibold border-2 ${theme===k?'border-amber-500':'border-transparent'}`} style={{background:grad||c,color:k==='black'||k==='gray'||k==='smooth'?'#ccc':'#333'}}>{l}</button>))}</div>
        </div>
        <div><label className="text-xs text-gray-400 mb-1 block">{t.language}</label>
          <div className="grid grid-cols-2 gap-2">{[['zh','中文'],['vi','Tiếng Việt']].map(([c,l])=>(<button key={c} onClick={()=>upd({lang:c})} className={`py-2.5 rounded-xl text-sm font-semibold ${form.lang===c?'bg-amber-600 text-white':'bg-white/[0.04] text-gray-500'}`}>{l}</button>))}</div>
        </div>
