...
function BackupSection({settings,t,onUpdate}){
  ...
  return(<div className="space-y-3 pt-4 border-t border-white/[0.06]"><h3 className="text-sm font-semibold text-amber-400">{t.backup}</h3>
    <div className="bg-emerald-500/[0.06] border border-emerald-500/20 rounded-xl p-3 space-y-2">
      <p className="text-sm font-semibold text-emerald-400">☁️ 雲端備份</p>
      <p className="text-[11px] text-gray-500 leading-relaxed">換手機前先備份，登入新手機後查詢就能直接還原</p>
      ...
      <label className="flex items-center gap-2 pt-1 cursor-pointer"><input type="checkbox" checked={!!settings.autoCloudBackup} onChange={e=>onUpdate&&onUpdate({...settings,autoCloudBackup:e.target.checked})} className="accent-emerald-500"/><span className="text-xs text-gray-400">每日第一次登入驗證密碼後，自動在背景備份一次</span></label>
    </div>
  </div>);
}
...
function CustomerPage({t,settings,onClose}){
  ...
  return(<div className="fi space-y-3 max-w-lg mx-auto px-4 py-2">
    ...
    {mode==='blacklist'&&(<div className="space-y-2">
      ...
      <p className="text-sm text-amber-300 text-center pt-2 leading-relaxed">符合條件才會出現在這個分頁：要在流水詳情輸入姓名、稱謂、手機，並勾選拒接及填寫原因</p>
    </div>)}
