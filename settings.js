...
function BackupSection({settings,t}){
  ...
  const doCloudBackup=async()=>{
    ...
    try{
      ...
      if(r&&r.ok)setCloudMsg({ok:true,text:'✓ 已備份到雲端（'+settings.year+'年）'});
  ...
  return(<div className="space-y-3 pt-4 border-t border-white/[0.06]"><h3 className="text-sm font-semibold text-amber-400">{t.backup}</h3>
    <div className="bg-emerald-500/[0.06] border border-emerald-500/20 rounded-xl p-3 space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <button onClick={doCloudCheck} disabled={cloudBusy} className="py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.1] text-gray-200 text-sm font-semibold disabled:opacity-50">查詢雲端備份</button>
      </div>
